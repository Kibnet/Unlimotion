...
internal static class TelegramCommandAuthorizationContract
{
    private const long AllowedUserId = 42;
    private const long UnauthorizedUserId = 404;
    private const long ChatId = 1001;

    public static async Task AssertUnauthorizedUserDoesNotSendMessagesOrQueryTasksAsync()
    {
    ...
    public static async Task AssertStartAndHelpReturnRussianCommandTextForAllowedUserAsync()
    {
        var responder = new RecordingResponder();
    ...
    public static async Task AssertSearchWithResultsShowsTaskListForAllowedUserAsync()
    {
        var query = new RecordingTaskQuery
        {
    ...
    public static async Task AssertSearchWithoutResultsSendsNotFoundMessageForAllowedUserAsync()
    {
        var responder = new RecordingResponder();
        var handler = CreateHandler(new RecordingTaskQuery(), responder);
    ...
    public static async Task AssertTaskCommandRoutesTaskIdToResponderForAllowedUserAsync()
    {
        var responder = new RecordingResponder();
        var handler = CreateHandler(new RecordingTaskQuery(), responder);
    ...
    public static async Task AssertRootWithResultsShowsRootTaskListForAllowedUserAsync()
    {
        var query = new RecordingTaskQuery
        {
    ...
    public static async Task AssertUnknownCommandReturnsHelpHintForAllowedUserAsync()
    {
        var responder = new RecordingResponder();
        var handler = CreateHandler(new RecordingTaskQuery(), responder);
    ...
    public static async Task<TelegramCommandAuthorizationScenarioResult> ExecuteSupportedCommandsScenarioAsync()
    {
        var unauthorizedQuery = new RecordingTaskQuery();
        var unauthorizedResponder = new RecordingResponder();
        var unauthorizedHandler = CreateHandler(unauthorizedQuery, unauthorizedResponder);

        await unauthorizedHandler.HandleMessageAsync(new TelegramCommandMessage(
            UnauthorizedUserId,
            "intruder",
            ChatId,
            "/search roadmap"));

        var commandQuery = new RecordingTaskQuery
        {
            SearchResults = [CreateTask("task-1", "Roadmap")],
            RootResults = [CreateTask("root-1", "Root task")]
        };
        var commandResponder = new RecordingResponder();
        var commandHandler = CreateHandler(commandQuery, commandResponder);

        await commandHandler.HandleMessageAsync(AllowedMessage("/start"));
        await commandHandler.HandleMessageAsync(AllowedMessage("/help"));
        await commandHandler.HandleMessageAsync(AllowedMessage("/search road"));
        await commandHandler.HandleMessageAsync(AllowedMessage("/task task-42"));
        await commandHandler.HandleMessageAsync(AllowedMessage("/root"));

        return new TelegramCommandAuthorizationScenarioResult(
            UnauthorizedUserBlocked:
                unauthorizedQuery.TotalCalls == 0 &&
                unauthorizedResponder.TotalCalls == 0,
            StartAndHelpSupported:
                commandResponder.Messages.Count == 2 &&
                commandResponder.Messages[0].Text == "Добро пожаловать! Введите /help для просмотра доступных команд." &&
                commandResponder.Messages[1].Text == "/search [запрос] - поиск задач\n/task [ID] - просмотр задачи\n/root - корневые задачи",
            SearchSupported:
                commandQuery.SearchCalls == 1 &&
                commandQuery.LastSearchQuery == "road" &&
                commandResponder.TaskLists.Count >= 1 &&
                commandResponder.TaskLists[0].MessageText == "Результаты поиска:\n" &&
                commandResponder.TaskLists[0].Results.Single().Title == "Roadmap",
            TaskCommandSupported:
                commandResponder.TaskRequests.Count == 1 &&
                commandResponder.TaskRequests[0] == (ChatId, "task-42"),
            RootCommandSupported:
                commandQuery.RootCalls == 1 &&
                commandResponder.TaskLists.Count >= 2 &&
                commandResponder.TaskLists[1].MessageText == "Корневые задачи:\n" &&
                commandResponder.TaskLists[1].Results.Single().Title == "Root task");
    }

    public static async Task AssertSupportedCommandsScenarioResultAsync(
        TelegramCommandAuthorizationScenarioResult result)
    {
        await Assert.That(result.UnauthorizedUserBlocked).IsTrue();
        await Assert.That(result.StartAndHelpSupported).IsTrue();
        await Assert.That(result.SearchSupported).IsTrue();
        await Assert.That(result.TaskCommandSupported).IsTrue();
        await Assert.That(result.RootCommandSupported).IsTrue();
    }
...
internal sealed record TelegramCommandAuthorizationScenarioResult(
    bool UnauthorizedUserBlocked,
    bool StartAndHelpSupported,
    bool SearchSupported,
    bool TaskCommandSupported,
    bool RootCommandSupported);
