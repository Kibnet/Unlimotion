...
public class StormTaskStatusCompletionBlockExecutableSpecTests
{
    [Test]
    public async Task TaskStatusCompletionBlockScenario_ExecutesFeatureSteps()
    {
        ...
        await Assert.That(scenario.Tags).Contains("@scenario:SC-0002-002");
        ...
        var runner = new StormScenarioRunner(TaskStatusCompletionBlockStepDefinitions.Create());
        var context = await runner.ExecuteAsync(scenario);

        var expectedStepDefinitionIds = new[] { "SD-0055", "SD-0056", "SD-0057", "SD-0058" };
