...
public sealed class SeamlessLoadingIndicator : Control
{
    // A seeded launch schedule repeats seamlessly without synchronized lap clocks.
    internal static readonly TimeSpan AnimationPeriod = TimeSpan.FromSeconds(24);
    ...
    public SeamlessLoadingIndicator()
    {
    ...
    internal double CurrentPhase { get; private set; }
    ...
    internal static double NormalizePhase(TimeSpan elapsed, TimeSpan period)
    {
        ...
        var ticks = elapsed.Ticks % period.Ticks;
        if (ticks < 0)
        {
            ticks += period.Ticks;
        }

        return (double)ticks / period.Ticks;
    }
    ...
    internal static bool ShouldAnimate(
        ...
        Size bounds) =>
        isAttached && isEffectivelyVisible && bounds.Width > 0 && bounds.Height > 0;
    ...
    private void EnsureDrawingResources()
    {
        ...
        _edgePen = new Pen(new SolidColorBrush(Color.Parse("#57545D")), 46, lineCap: PenLineCap.Flat);
    ...
    private void OnAnimationFrame(TimeSpan timestamp)
    {
        ...
        if (!ShouldAnimate(_isAttached, IsEffectivelyVisible, Bounds.Size))
        {
        ...
        CurrentPhase = NormalizePhase(timestamp - _animationStart, AnimationPeriod);
        InvalidateVisual();
        RequestNextFrame();
    }
    ...
    {
        if (_isFramePending || !ShouldAnimate(_isAttached, IsEffectivelyVisible, Bounds.Size))
        {
            return;
        }
