...
Команда: `/storm:bdd-sync` after `/storm:bdd-implement SC-0014-003 executable step definitions`
...
| Scenario -> Step Definition links | 4/45 |
| Новые связи | `SC-0014-003 -> TS-0029`; `SC-0014-003 -> SD-0013..SD-0016` |
| Draft scenarios | нет |
...
| --- | --- | --- |
| step_definitions | Step definitions есть для `SC-0015-002`, `SC-0014-002`, `SC-0014-001` и `SC-0014-003`; repo-local runner intentionally covers selected slices only. | Расширять по отдельным high-value scenarios, не генерировать placeholders. |
...
`SC-0014-003` получил repo-local executable step-definition slice, а `SC-0014-001`, `SC-0014-002` и `SC-0015-002` сохранили уже реализованные executable связи. Android/iOS build smoke остаются environment blocker, а не product failure. `CV-0007` намеренно остается вне active Gherkin scenario/test links после Варианта B.
