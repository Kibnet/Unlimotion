...
Команда: `/storm:bdd-sync` after `/storm:bdd-implement SC-0015-002 executable step definitions`
...
| Scenario -> Step Definition links | 1/45 |
| Новые связи | `SC-0015-002 -> TS-0026`; `SC-0015-002 -> SD-0001..SD-0004` |
| Draft scenarios | нет |
...
| --- | --- | --- |
| step_definitions | Step definitions есть только для `SC-0015-002`; repo-local runner intentionally covers first slice only. | Расширять по отдельным high-value scenarios, не генерировать placeholders. |
...
`SC-0015-002` получил первый repo-local executable step-definition slice. Android/iOS build smoke остаются environment blocker, а не product failure. `CV-0007` намеренно остается вне active Gherkin scenario/test links после Варианта B.
