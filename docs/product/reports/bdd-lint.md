...
Команда: `/storm:bdd-lint` after `/storm:cover CV-0007 attachment workflow confirmation`
...
| Declarative language | PASS | No new Gherkin text was added for unconfirmed attachment workflow. |
| Step definitions | WARNING | `step_definitions` remains empty by design; TUnit tests carry executable evidence. |
| CV-0007 product claim | PASS | Attachment code is retained as candidate, not promoted to active story/scenario without product decision. |
...
4. `CV-0007` remains blocked pending product decision; no scenario/test link should be inferred from attachment code alone.
