...
| --- | --- | --- |
| ST-0014 | `AC-0039` сохраняет coverage `full`: command/auth покрыт `TS-0022`, а `SC-0014-001` исполняется через `SD-0009..SD-0012` и `TS-0028`. `AC-0040` сохраняет coverage `critical`: callback behavior покрыт `TS-0023`, `SC-0014-003` исполняется через `SD-0013..SD-0016` и `TS-0029`, Git timer conflict-safety покрыт `TS-0025`, а `SC-0014-002` исполняется через `SD-0005..SD-0008` и `TS-0027`. Реальный Telegram polling/network остаётся вне scope. | `features/storm/st-0014-telegram-bot.feature`, `src/Unlimotion.Test/StormTelegramCallbackExecutableSpecTests.cs`, `src/Unlimotion.Test/StormBdd/TelegramCallbackStepDefinitions.cs`, `TS-0023`, `TS-0029` |
...
| --- | --- | --- |
| ST-0014 / AC-0039, AC-0040 | Нет active gap; все три `ST-0014` scenarios теперь step-executable. | Поддерживать `TS-0022`, `TS-0028`, `TS-0023`, `TS-0029`, `TS-0025`, `TS-0027` при изменениях Telegram bot. |
...
| BDD execution | Step definitions покрывают `SC-0015-002`, `SC-0014-002`, `SC-0014-001` и `SC-0014-003`. | Расширять executable step definitions отдельными SPEC по high-value scenarios; не создавать placeholder steps массово. |
