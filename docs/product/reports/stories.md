...
| --- | --- | --- |
| ST-0014 | `AC-0039` сохраняет coverage `full`: command/auth покрыт `TS-0022`, а `SC-0014-001` исполняется через `SD-0009..SD-0012` и `TS-0028`. `AC-0040` сохраняет coverage `critical` через `TS-0023`, `TS-0025`, `TS-0027`. Реальный Telegram polling/network остаётся вне scope. | `features/storm/st-0014-telegram-bot.feature`, `src/Unlimotion.Test/StormTelegramCommandExecutableSpecTests.cs`, `src/Unlimotion.Test/StormBdd/TelegramCommandStepDefinitions.cs`, `TS-0022`, `TS-0028` |
...
| --- | --- | --- |
| ST-0014 / AC-0039, AC-0040 | Нет active gap. | Поддерживать `TS-0022`, `TS-0028`, `TS-0023`, `TS-0025`, `TS-0027` при изменениях Telegram bot. |
...
| BDD execution | Step definitions покрывают только `SC-0015-002`, `SC-0014-002` и `SC-0014-001`. | Расширять executable step definitions отдельными SPEC по high-value scenarios; не создавать placeholder steps массово. |
