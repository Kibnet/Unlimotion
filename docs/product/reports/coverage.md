...
Эта итерация добавляет первый executable BDD slice для `ST-0015 / AC-0042 / SC-0015-002`: сценарий исполняется из `.feature` текста через repo-local step definitions `SD-0001..SD-0004` и TUnit evidence `TS-0026`.
...
| Passing scenarios | 7 |
| Step definitions | 4 |
| Step-executable scenarios | 1/45 |

## Результат SC-0015-002 Executable Slice

| Item | Было | Стало | Evidence |
| --- | --- | --- | --- |
| `SC-0015-002.step_definitions` | `[]` | `SD-0001..SD-0004` | `StormPlatformShellExecutableSpecTests` исполняет шаги из `features/storm/st-0015-platform-shells.feature`. |
| `CV-0005 / AC-0042` | project-contract tests + Browser build smoke | project-contract tests + Browser build smoke + executable step-definition slice | `TS-0024` покрывает project contracts; `TS-0026` покрывает executable BDD path. |
...
| Step definitions | 4 |
| Step-executable scenarios | 1/45 |
| Executable specification ratio | 1/45 step-executable; 7/45 passing scenarios |
...
| `dotnet test src/Unlimotion.Test/Unlimotion.Test.csproj -c Release --no-build --no-restore -- --treenode-filter "/*/*/StormPlatformShellExecutableSpecTests/*" --output Detailed` | прошло 1/1 |
| `dotnet test src/Unlimotion.Test/Unlimotion.Test.csproj -c Release --no-build --no-restore -- --treenode-filter "/*/*/PlatformShellProjectContractTests/*" --output Detailed` | прошло 3/3 |

## Оставшиеся Gaps

1. Step definitions покрывают только `SC-0015-002`: остальные scenarios пока rely on linked TUnit evidence.
