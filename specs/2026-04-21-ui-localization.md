...
| EXEC | Регрессия пустого выбора языка в ComboBox | 0.88 | Нет | Передать итог пользователю | Нет | Да, пользователь сообщил пустой selection после выбора языка | Причина: `LanguageOptions` заменялся новым списком при смене культуры, и `ComboBox` терял selection; коллекция сделана стабильной, локализованные подписи обновляются внутри существующих элементов, добавлен item template и regression test. `Unlimotion.Test.csproj` собирается; полный sequential test run дал 183/184 passed, единственный fail в unrelated `BackupViaGitServiceTests.ConnectRepository_RefusesNonEmptyRemoteAndNonEmptyLocalFolderWithoutConfirmation`; полный solution build в текущем окружении блокируется отсутствующим workload `maccatalyst` для mobile projects | `src/Unlimotion.ViewModel/SettingsViewModel.cs`, `src/Unlimotion.ViewModel/Localization/LanguageOption.cs`, `src/Unlimotion/Views/SettingsControl.axaml`, `src/Unlimotion.Test/SettingsViewModelTests.cs`, `specs/2026-04-21-ui-localization.md` |
| EXEC | Исправления по review ветки | 0.9 | Нет | Передать итог пользователю | Нет | Да, пользователь попросил исправить review findings | Фильтры `UnlockedTimeFilters`/`DurationFilters` больше не заменяются при смене языка и сохраняют активные состояния; `TaskRelationPickerViewModel` отписывается от `CultureChanged`; оставшийся folder dialog title вынесен в `.resx`; добавлены regression tests. `Unlimotion.Test.csproj` собран, targeted tests `LocalizationDisplayDefinitionTests`, `SettingsViewModelTests`, `LocalizationSettingsTests` прошли; полный test run по-прежнему блокируется unrelated async crash `TaskRelationsIndex` | `src/Unlimotion.ViewModel/MainWindowViewModel.cs`, `src/Unlimotion.ViewModel/DurationFilter.cs`, `src/Unlimotion.ViewModel/UnlockedTimeFilter.cs`, `src/Unlimotion.ViewModel/TaskRelationPickerViewModel.cs`, `src/Unlimotion/Views/MainControl.axaml.cs`, `src/Unlimotion.ViewModel/Resources/*`, `src/Unlimotion.Test/LocalizationDisplayDefinitionTests.cs`, `specs/2026-04-21-ui-localization.md` |
