...
| EXEC | Auto-select branch and remote after connect | 0.9 | Нет | Передать результат пользователю | Нет | Нет | Добавлен авто-выбор валидного remote source и branch после Git metadata reload, а bootstrap existing remote теперь переключает устаревший `PushRefSpec` на фактическую ветку remote; `dotnet build src\Unlimotion.Test\Unlimotion.Test.csproj --no-restore /clp:ErrorsOnly`, `dotnet build src\Unlimotion\Unlimotion.csproj --no-restore /clp:ErrorsOnly` and `dotnet test src\Unlimotion.sln --no-build` pass (`167/167`). Full solution build blocked by running `Unlimotion.Desktop.exe` locking desktop output DLLs. | `src/Unlimotion.ViewModel/SettingsViewModel.cs`, `src/Unlimotion/Services/BackupViaGitService.cs`, `src/Unlimotion.Test/SettingsViewModelTests.cs`, `src/Unlimotion.Test/BackupViaGitServiceTests.cs`, `specs/2026-04-21-first-run-git-backup-bootstrap.md` |
| EXEC | Ignore migration report files during Git connect | 0.9 | Нет | Передать результат пользователю | Нет | Нет | `migration.report` и `availability.migration.report` больше не считаются локальным содержимым при preview, добавляются в `.gitignore`, исключаются из baseline commit и удаляются из Git tracking/remote tree при подключении существующего repository; `dotnet build src\Unlimotion.Test\Unlimotion.Test.csproj --no-restore /clp:ErrorsOnly`, `dotnet build src\Unlimotion\Unlimotion.csproj --no-restore /clp:ErrorsOnly` and `dotnet test src\Unlimotion.sln --no-build` pass (`169/169`) | `src/Unlimotion/Services/BackupViaGitService.cs`, `src/Unlimotion.Test/BackupViaGitServiceTests.cs`, `specs/2026-04-21-first-run-git-backup-bootstrap.md` |
| EXEC | Prevent checkout conflict on migration reports | 0.9 | Нет | Передать результат пользователю | Нет | Да, пользователь подтвердил ручным тестом, что предыдущий cleanup был поздним | Перед checkout/merge локальные report-файлы временно удаляются и затем восстанавливаются как ignored/untracked, поэтому Git больше не блокируется конфликтом untracked files; добавлен regression test для remote+local report-файлов; `dotnet build src\Unlimotion.Test\Unlimotion.Test.csproj --no-restore /clp:ErrorsOnly`, `dotnet build src\Unlimotion\Unlimotion.csproj --no-restore /clp:ErrorsOnly /nr:false` and `dotnet test src\Unlimotion.sln --no-build` pass (`170/170`) | `src/Unlimotion/Services/BackupViaGitService.cs`, `src/Unlimotion.Test/BackupViaGitServiceTests.cs`, `specs/2026-04-21-first-run-git-backup-bootstrap.md` |
| EXEC | Discard reports for effectively empty local folder | 0.9 | Нет | Передать результат пользователю | Нет | Да, пользователь уточнил, что если были только report-файлы, восстанавливать их не нужно | В checkout-flow local-only report-файлы больше не восстанавливаются, а remote report-файлы после cleanup также удаляются из working tree; merge-flow с реальными локальными задачами продолжает сохранять локальные report-файлы; `dotnet build src\Unlimotion.Test\Unlimotion.Test.csproj --no-restore /clp:ErrorsOnly` and `dotnet test src\Unlimotion.sln --no-build` pass (`170/170`) | `src/Unlimotion/Services/BackupViaGitService.cs`, `src/Unlimotion.Test/BackupViaGitServiceTests.cs`, `specs/2026-04-21-first-run-git-backup-bootstrap.md` |
