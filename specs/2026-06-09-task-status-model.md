...
| EXEC | Регрессионные UI-замечания по статусам | 0.82 | Нужно подтвердить headless UI tests после правки | Исправить локальное время, доступные пункты flyout, реактивную прозрачность и единый отступ статусной иконки | Нет | Да, пользователь перечислил четыре конкретных замечания | Замечания находятся в пределах утвержденной статусной модели и требуют локального bugfix с усилением UI coverage | `TaskItemViewModel.cs`, `TaskStatusPicker.cs`, `MainControl.axaml`, `GraphControl.axaml`, `MainControlTaskStatusIconUiTests.cs` |
| EXEC | Валидация регрессионных правок статусов | 0.86 | Полный serial-прогон сохраняет shared-state flakiness в старых UI-сценариях; изолированные повторы зелёные | Завершить отчет без коммита | Нет | Нет | Исправления покрыты targeted UI/domain tests; полный прогон дошёл до 514/517 passed, три упавших non-status теста прошли отдельно | `MainControlTaskStatusIconUiTests.cs`, `MainControlAvailabilityUiTests.cs`, `TaskStatusTransitionTests.cs`, `MainWindowViewModelTests.cs`, `TaskItemViewModel.cs`, `TaskStatusPicker.cs`, `MainControl.axaml`, `GraphControl.axaml` |
| EXEC | Полировка отступов временных метаданных в списках | 0.88 | Нет runtime screenshot artifact после микроправки | Завершить отчет без коммита | Нет | Да, пользователь показал скриншоты с прилипанием даты/elapsed к названию | Добавлен явный right margin для date/time labels во вкладках и UI-тест на spacing во вкладке `Выполняется` | `MainControl.axaml`, `MainControlTaskStatusIconUiTests.cs` |
| EXEC | Полировка completed-иконки под стандартный CheckBox | 0.90 | Screenshot artifact не создан; визуальный контракт закреплен headless UI assertions | Завершить отчет без коммита | Нет | Да, пользователь попросил сделать `Выполнено` визуально идентичным чекбоксу | `TaskStatusIcon.Completed` переведен на checked `CheckBox` resources, `ControlCornerRadius` и `CheckMarkPathData`; тест сверяет размер, рамку, фон, glyph brush/geometry и placement с runtime `CheckBox` в Light/Dark | `TaskStatusIcon.cs`, `MainControlTaskStatusIconUiTests.cs` |
| EXEC | Исправление GitHub Actions failures | 0.86 | Нужно дождаться remote rerun после push | Закоммитить и отправить CI-fix | Нет | Да, пользователь попросил исправить ошибки GitHub Actions | По логам PR #257 исправлены async date-filter assertion, Avalonia.Headless dispose flake и Android restore failure из-за revoked ReactiveUI/Splat author-signature на Linux runner | `.github/workflows/android-packaging.yml`, `MainWindowViewModelTests.cs`, `RoadmapGraphUiTests.cs` |
