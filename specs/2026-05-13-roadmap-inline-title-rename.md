...
| EXEC | Wrapped roadmap title editor fix | 0.97 | Нет | Запушить обновление в PR | Нет | Да, пользователь сообщил follow-up дефект многострочного title editor | `TextBox` roadmap inline rename получил `TextWrapping=Wrap`, высоту не меньше исходного wrapped title и `MaxWidth` как у title text; UI-тест теперь проверяет многострочный title и высоту editor; прошли build, targeted inline rename test и `RoadmapGraphUiTests` 35/35 | `src/Unlimotion/Views/GraphControl.axaml.cs`, `src/Unlimotion.Test/RoadmapGraphUiTests.cs`, `specs/2026-05-13-roadmap-inline-title-rename.md` |
