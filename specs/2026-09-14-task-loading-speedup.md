...
Расширение того же исправления изоляции: поиск всех непосредственных new MainWindowViewModel/SettingsViewModel/MainWindowViewModelFixture выявил также MainWindowViewModelFixtureLifecycleTests и SettingsAppearanceContractTests без SharedUiStateParallelLimit. Добавляется class-level limiter этим двум классам. Helper *UiContract вызываются из уже защищённых executable spec классов. Первый полный прогон419a5f55 остановлен для пересборки с этим уточнением, его нельзя считать финальным evidence.

Post-EXEC дополнительного sweep: advisory reviewer подтвердил limiter для двух классов; все11 *UiContract вызываются через защищённые Storm*ExecutableSpecTests/ToastNotificationUiTests. Новых findings нет. Assertions, конкурентные операции внутри lifecycle-тестов и production-код сохранены. Build Debug:0 errors,90 existing warnings. Runtime acceptance проверяется новым полным прогоном без фильтра и новым CI; остановленный ранний прогон не evidence успеха.

### 22.2 Повтор локального emoji failure

На полном локальном прогоне91327180 повторился тот же ParentEmojiTrail visibility failure, теперьwidth360. Language и binding tests прошли; trace не содержит protected overlaps. Прогон остановлен после воспроизведения для диагностики, итоговым green не считается. Гипотеза: ConfigureLongEmojiAncestorChain подменяет только VM-relations, хотя fixture использует настоящее файловое хранилище и тестовый autosave10ms. PlannedBeginDateTime в helper изменяется через editable property; последующее применение сохранённого графа может удалить искусственные неперсистированные связи. Проверка: принудительно выполнить Update между настройкой цепочки и UI assertions. Если red подтверждён, создавать цепочку в исходных файлах fixture до Connect, сохранив UI assertions и принудительное обновление. Production не меняется; это стабилизация test setup в рамках исправления PR checks.

Проверка22.2: принудительный taskRepository.Update после старой VM-only подготовки дал3/3fail на исходном visibility assertion (360/390/430), `emoji-forced-red.log`. После записи согласованного графа в fixture input до Connect весь MainControlTaskCardLayoutUiTests23/23green, `emoji-green.log`; forcedUpdate сохранён. Удалены прежние обратные Contains-связи; новые8ancestorузлов согласованы в обе стороны. Исходные snapshots и production не менялись. Advisory post-EXEC Scope/Contract/Adversarial + tester/UX/developer passes: находок нет. Это изменение только test setup/регрессии, визуальный before/after product artifact не применим; evidence — реальные headless layout/assertions и TRX. Полный local/CI результат будет указан в PR.
