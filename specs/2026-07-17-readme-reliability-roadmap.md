...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены, draft Stage-3 PR #280 открыт. `ffd5ae6` снял AVD-root barrier; `b1ebf24` сделал readiness observable и bounded. Replacement `30006652106` прошёл scope/contract/Windows/macOS/Android dual-APK build, Linux сохранил expected `NU3012`; API 23 и API 36 оба достигли `adb_state=device`, `sys.boot_completed=1` и установки exact APK. Оба затем timeout-нулись на блокирующем post-boot `am start -W`, не на boot/cleanup. Новый uncommitted narrow fix убирает только `-W`, сохраняя wait/PID/fatal-logcat gate; static RED, dynamic hanging-`-W` fixture и Android regression PASS 71.2 s, `All` PASS 182 negatives за 85.0 s. Fresh Tests `30006652135` PASS Unit 830/830 и Headless 36/36, поэтому 12 прошлых layout failures классифицированы как неповторившийся flake. NuGet prerequisite остаётся `NEEDS-FIX`; final reset/native/merge и stages 4-10 pending
...
- Evidence inspected: historical matrices сохраняют diagnostic trace. Run `29838807057` на `d0c68dd5` подтвердил scope/contract, Windows, macOS x64/arm64 и Android dual-APK build PASS; Linux expected `NU3012`, CodeQL `29838806826` и AndroidPkg `29838805889` PASS. Ubuntu 22.04 API23/API36 успешно установили/проверили `libpulse0`, image и exact APK, но preserved logs обоих jobs назвали `Unknown AVD name`. Run `30003714447` на `ffd5ae6` связал create/launch; `b1ebf24` run `30006652106` добавил exact readiness observations: оба API достигли `device` и `sys.boot_completed=1`, exact APK install, после чего `am start -W` был принудительно завершён 30-s timeout. Validator теперь запускает activity без `-W`, а retained 15-s wait + live PID + fatal-free logcat остаются launch proof. Static RED, hanging-`-W` success fixture, Android regression PASS 71.2 s и `All` PASS 182/85.0 s. Tests `30006652135` PASS Unit 830/830 + Headless 36/36; предыдущий 12-failure UI result classified non-reproducible. Review/replacement CI и full exact-SHA reset pending
