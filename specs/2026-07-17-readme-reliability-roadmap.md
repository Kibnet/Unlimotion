...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены, draft Stage-3 PR #280 открыт. Commit `1ef54245` перевёл emulator jobs на совместимый Ubuntu 22.04 и сделал loader diagnostic прозрачным: run `29836756606` дал contract/Windows/macOS/Android dual-APK build PASS и expected Linux `NU3012`, затем оба API jobs точно сообщили missing `libpulse.so.0`. Новый per-job bounded APT contract устанавливает native `libpulse0` без recommendations и проверяет package state/exact SONAME; TDD RED 2.3 s, Android regression PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent review/reruns PASS (Android 51.8 s, `All` 182/91.4 s). AndroidPkg/CodeQL PASS; Tests подтвердил Unit `830/830` и Headless `36/36`. NuGet prerequisite остаётся `NEEDS-FIX`; final reset/native/merge и stages 4-10 pending
...
- Decision: stages 1-2 завершены. Stage 3 продолжает только approved harness/evidence/README truthfulness work; `Program.cs` first-run fix передан Stage 5. Ubuntu 22.04 снял `libtiff.so.5` barrier, transparent replacement выявил следующий exact host dependency `libpulse.so.0`; установить его native Jammy package стандартным bounded APT pattern, закрепить per-job regression и повторить review/Distribution/Tests. Final acceptance всё равно сбрасывается после dependency-security prerequisite. В `rxs1` сначала закрыть все current review findings и получить повторные Role-Based/Post-SPEC PASS; затем отдельное точное approval, spec commit/implementation PR/merge и PR #280 rebase/full reset. Stage 4 закрыт до Stage-3 merge
- Review passes:
...
- Evidence inspected: historical matrices сохраняют diagnostic trace. Run `29836756606` на `1ef54245` подтвердил scope/contract, Windows, macOS x64/arm64 и Android dual-APK build PASS; Linux expected `NU3012`. Ubuntu 22.04 API23/API36 прошли emulator PATH и прежний opaque `127`; diagnostic helper в обоих jobs показал `/usr/local/lib/android/sdk/emulator/qemu/linux-x86_64/qemu-system-x86_64: error while loading shared libraries: libpulse.so.0`. Host-runtime step использует существующий 3-attempt APT pattern, `--no-install-recommends libpulse0`, package-state и exact SONAME guards; TDD RED 2.3 s, Android regression PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent review/reruns PASS (Android 51.8 s, `All` 182/91.4 s), `git diff --check` PASS. AndroidPkg `29836756635` и CodeQL `29836756689` PASS; Tests `29836756212` подтвердил Unit `830/830` и Headless `36/36`. Replacement CI и full exact-SHA reset pending
...
| MEDIUM | stage 3 / Android emulator libpulse runtime | Run `29836756606` в API23/API36 точно сообщил missing `libpulse.so.0` из qemu loader на Ubuntu 22.04 | Установить native `libpulse0` без recommendations через bounded APT retry; проверить package state и `/usr/lib/x86_64-linux-gnu/libpulse.so.0` в каждом job | fixed in working tree; TDD RED 2.3 s, Android 43.5 s и `All` 182/103.8 s PASS; independent Android 51.8 s + `All` 182/91.4 s/review PASS; run pending |
...
- Checks rerun: run `29836756606` подтвердил Ubuntu 22.04/probe fix и native Windows/macOS/Android build; оба API jobs точно fail-closed на missing `libpulse.so.0`. Host-runtime regression дал expected RED 2.3 s; bounded APT/package/SONAME guards — Android PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent Android 51.8 s и `All` 182/91.4 s, review PASS. Replacement Distribution pending. Любой tracked commit сбрасывает прежний full Unit/Headless/native evidence; final exact-SHA reset ещё не выполнен
- Validation evidence: stage 1 — PASS/merged; Stage 2 — PASS/merged; Headless prerequisite — PASS/merged PR #279; Stage 3 — runs `29794200613`, `29821815902`, `29822419518`, `29824937816`, `29827483394`, `29829779408`, `29831446698`, `29834142027`, `29836756606`. Latest companions: AndroidPkg `29836756635` и CodeQL `29836756689` PASS; Tests `29836756212` подтвердил Unit `830/830` и Headless `36/36`. Libpulse fix targeted + `All` 182 + independent review PASS; commit/replacement pending. `S3-AC-20` ждёт NuGet prerequisite, clean-archive/full-test/native evidence; final aggregate/review/merge pending; stages 4-10 pending
