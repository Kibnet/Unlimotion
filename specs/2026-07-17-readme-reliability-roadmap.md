...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены, draft Stage-3 PR #280 открыт. Commit `ffd5ae6` устранил producer/consumer AVD-root разрыв: replacement run `30003714447` прошёл scope/contract/Windows/macOS/Android dual-APK build, Linux сохранил expected `NU3012`; API 36 нашёл AVD и оба clean attempts завершили внутренний boot за 54.9/56.9 с. Затем выявлены два harness defects: readiness ошибочно требовал optional `init.svc.bootanim=stopped`, а отдельные ADB/cleanup calls могли пережить boot deadline; API 23 остался в том же шаге без итогового evidence, поэтому run был отменён. Новый validator принимает `sys.boot_completed=1`, записывает ADB state/properties в attempt log и ограничивает ADB/`avdmanager`/teardown timeout-ами. Local Android regression PASS 63.4 s, `All` PASS 182 negatives за 77.9 s. Companion Tests `30003714491` упал на 12 `MainControlTaskCardLayoutUiTests`; diff не затрагивает UI, повторный isolated job нужен до классификации. NuGet prerequisite остаётся `NEEDS-FIX`; final reset/native/merge и stages 4-10 pending
...
- Review passes:
  ...
  - Android readiness/ADB-boundedness remediation: run `30003714447` на PR candidate `ffd5ae6` доказал, что AVD isolation работает (API 36 launches booted twice), но strict `bootanim=stopped` дал ложный boot failure, а API 23 мог зависнуть внутри unbounded ADB/cleanup. RED readiness contract и hanging-ADB fixture; final fixture принимает `sys.boot_completed=1` при `bootanim=running`, сохраняет ADB diagnostics и завершает two-attempt failure with evidence within 12 s. Bash + Android PASS 63.4 s, `All` 182/77.9 s; replacement/review pending.
...
- Evidence inspected: historical matrices сохраняют diagnostic trace. Run `29838807057` на `d0c68dd5` подтвердил scope/contract, Windows, macOS x64/arm64 и Android dual-APK build PASS; Linux expected `NU3012`, CodeQL `29838806826` и AndroidPkg `29838805889` PASS. Ubuntu 22.04 API23/API36 успешно установили/проверили `libpulse0`, image и exact APK, но preserved logs обоих jobs назвали `Unknown AVD name`: `avdmanager create` не был связан с поиском `emulator`. Run `30003714447` на PR candidate `ffd5ae6` подтвердил, что AVD isolation связала create/launch: API 36 booted twice за 54.9/56.9 s, но silent ADB polling ошибочно отверг ready device и API 23 не вышел из launch step; run намеренно отменён как диагностически негодный. Validator теперь пишет `adb_state`, `sys.boot_completed` и `init.svc.bootanim` в attempt log, принимает первый сигнал как readiness, использует 30-s bounded ADB/`avdmanager` calls и ограниченную teardown escalation. Android regression PASS 63.4 s, `All` PASS с 182 negatives за 77.9 s. Tests `30003714491` = 818/830, 12 `MainControlTaskCardLayoutUiTests` failures; UI diff отсутствует, required isolated rerun pending. Review/replacement CI и full exact-SHA reset pending
...
| HIGH | stage 3 / Android readiness and teardown | Run `30003714447`: API 36 дважды внутренне booted, но silent gate требовал optional `init.svc.bootanim=stopped`; API 23 остался в ADB/cleanup path дольше boot deadline | Gate только `sys.boot_completed=1`; писать ADB diagnostics в attempt log; bounded timeout для всех runtime ADB/`avdmanager` commands и escalation emulator teardown; добавить hanging-ADB + `bootanim=running` fixtures | fixed locally; RED/Green Android regression 63.4 s и `All` 182/77.9 s PASS; review/replacement pending |
| MEDIUM | CI / unrelated UI suite | Companion Tests `30003714491` дал 12 failures `MainControlTaskCardLayoutUiTests` (818/830), тогда как текущий diff меняет только Android harness/tests/spec | Rerun isolated Tests job; только повторяемый failure эскалировать в отдельный UI scope/spec | pending classification |
...
- Checks rerun: run `29838807057` подтвердил libpulse runtime/image/candidate barriers в обоих API jobs, после чего оба fail-closed точно на unknown AVD name. `ffd5ae6` fixed AVD-root; run `30003714447` получил API 36 boot evidence (54.9/56.9 s), затем показал overly strict readiness и unbounded ADB teardown; API 23 hang был остановлен отменой workflow. Readiness/timeout TDD: static RED, `bootanim=running` success fixture и hanging-ADB bounded failure fixture; Bash + Android PASS 63.4 s и `All` 182/77.9 s. Review и replacement Distribution pending. Любой tracked commit сбрасывает прежний full Unit/Headless/native evidence; final exact-SHA reset ещё не выполнен
- Validation evidence: stage 1 — PASS/merged; Stage 2 — PASS/merged; Headless prerequisite — PASS/merged PR #279; Stage 3 — runs `29794200613`, `29821815902`, `29822419518`, `29824937816`, `29827483394`, `29829779408`, `29831446698`, `29834142027`, `29836756606`, `29838807057`, `30003714447` (cancelled after diagnostics). Latest companion Tests `30003714491` = 818/830 with 12 UI layout failures pending isolated rerun; CodeQL `30003714459` and AndroidPkg `30003714436` were green before cancellation. Readiness/timeout fix Bash + Android + `All` 182 PASS; review/commit/replacement pending. `S3-AC-20` ждёт NuGet prerequisite, clean-archive/full-test/native evidence; final aggregate/review/merge pending; stages 4-10 не стартованы
