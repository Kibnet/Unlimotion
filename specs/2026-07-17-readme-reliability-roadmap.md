...
- Blocking dependency-security prerequisite после первого native run: `specs/2026-07-21-reactiveui-signature-chain-remediation.md`; требует отдельного approval/PR/merge, затем полного reset Stage-3 evidence.
...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены. Draft Stage-3 PR #280 открыт. Distribution run `29794200613` подтвердил scope/contract и Windows на elevated runner, выявил Android local-feed deletion, остановил Linux до smoke на revoked NuGet signature chain и остановил macOS на первом portable launch в `/Tasks`; cross-platform first-run impact для Windows установлен последующим source analysis, а не Windows failure. Harness и truthful README workaround fixes реализованы в working tree. NuGet prerequisite пока существует только как untracked draft в отдельном worktree `rxs1`: Role-Based/Post-SPEC final re-reviews PASS; отдельное точное approval, spec commit, implementation PR и merge ещё pending; final reset/native/merge и stages 4-10 pending
...
- Decision: stages 1-2 завершены. Stage 3 продолжает только approved harness/evidence/README truthfulness work; `Program.cs` first-run fix передан Stage 5. NuGet trust-chain untracked draft в отдельном `rxs1` прошёл Role-Based/Post-SPEC PASS; следующий gate — отдельное точное approval, после которого разрешены spec commit/implementation PR/merge. После prerequisite merge выполнить PR #280 rebase/full reset. Stage 4 закрыт до Stage-3 merge
- Review passes:
  ...
  - Stop decision: Stage-2 delivery и исходные Stage-3/LF/build-isolation approval gates закрыты; build-isolation implementation выполнена. Новый NuGet prerequisite прошёл Role-Based/Post-SPEC PASS и остановлен перед отдельным approval/delivery; только после его merge разрешены clean final-candidate reset и повторный native gate. API-23 stop rule остаётся условным.
- Evidence inspected: прежнее local/build evidence плюс четыре раздельных PR #280 workflow run: distribution `29794200613` — `distribution-scope`, `distribution-contract` и elevated `windows-x64-native` PASS; companion Tests `29794200612`, AndroidPkg `29794200679` и CodeQL `29794200684` — PASS. В distribution run macOS x64/arm64 завершили candidate build, ad-hoc sealing и evidence envelope, но validation остановилась на первом portable launch в `/Tasks` до Setup install и `pkgutil` signature classification; последующие package/sign classifications не выполнялись. Standalone Android потерял tracked Nodify и сделал public fallback с NU1603, тогда как отдельный AndroidPkg run сохранил local package и прошёл. Linux restore упал с `NU3012 Revoked` до начала smoke. Windows PASS получен на elevated runner; разрешение default в корень текущего диска (например, `C:\Tasks`) подтверждено source analysis и этим PASS не проверено. Current harness checks: полный static All PASS с 172 negative fixtures, включая WorkflowSecurity 69 negatives и Evidence 46 negatives; Android script suite PASS за 55.4 s; full exact-SHA reset pending
...
- No-findings justification: stages 1-2 PASS после fixes; Stage-3 deterministic build-isolation findings закрыты и implementation reviews PASS. `S3-AC-20` остаётся открыт на reset/full/native/delivery evidence; Stage-3 final PASS не выставлен до полного reset gate, native matrix, final independent review и merge
...
| HIGH | stage 3 / Android local feed | Standalone builder удалял tracked Nodify package; простое сохранение ignored feed допускало stale/substituted exact versions, hierarchical cache и dirty-source relabeling | Output-scoped flat feed из verified HEAD Nodify + current LibGit only; exact source mapping и post-restore SHA-512/source proof; recursive no-extra/symlink closure; sourceSha=clean HEAD/index/submodules; executable negatives | fixed in working tree; Android suite PASS 55.4 s, native rerun pending |
...
| BLOCKER | stage 3 / dependency security | Fresh Linux restore отклоняет revoked ReactiveUI/Splat chain до smoke; existing packaging flags disable verification | Получить exact approval уже reviewed-PASS spec в отдельном `rxs1`, затем выполнить spec commit/implementation PR/merge и только после merge rebase/full Stage-3 reset | draft untracked; reviews PASS, approval/commit/PR/merge pending |
...
- Checks rerun: прежние gates остаются baseline; на текущих harness/README fixes полный static All PASS с 172 negative fixtures за 95.6 s, включая WorkflowSecurity 69 negatives и Evidence 46 negatives; Android suite PASS за 55.4 s, paired README contract и parsers PASS. Любой tracked commit сбрасывает прежний full Unit/Headless/native evidence; final exact-SHA reset ещё не выполнен
- Validation evidence: stage 1 — PASS/merged; Stage 2 — PASS/merged; Headless prerequisite — local/CI/review PASS и merged PR #279; Stage 3 — build-isolation deterministic fix/reviews PASS и первый native matrix `29794200613` зафиксировал blockers, `S3-AC-20` ждёт clean-archive/full-test/native evidence; final native rerun/aggregate green, final review и merge pending; stages 4-10 — pending
...
| EXEC | Исправить Android feed, configured-smoke evidence и README overclaim | 0.99 | Full reset/native rerun | Review, commit and push exact harness/README diff | Нет | Не применимо | HEAD-bound isolated two-package feed, seeded-storage disclosure и EN/RU explicit-writable-path caveat закрывают harness/docs defects без runtime/data-path изменения; `/Tasks` и current-drive-root product fix передан Stage 5 | Stage-3 scripts/tests/README/spec |
| SPEC | Подготовить dependency-security prerequisite | 1.00 | Separate exact approval | Запросить `Спеку подтверждаю`, затем commit/implementation PR/merge | Да | Ещё не обращались | Validation, security и architect/governance final re-reviews PASS; revoked signature нельзя обходить `false/offline`; draft изолирован в `rxs1` | `specs/2026-07-21-reactiveui-signature-chain-remediation.md` в `rxs1`, reviewer verdicts |
