...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены, draft Stage-3 PR #280 открыт. `ffd5ae6` снял AVD-root barrier; `b1ebf24` сделал readiness observable и bounded; `1156eb7` удалил blocking `am start -W`, сохранив wait/PID/fatal-logcat gate. Replacement `30007920350` прошёл scope/contract/Windows/macOS/Android dual-APK build, Linux сохранил expected `NU3012`; API 23 полностью launch PASS, а API 36 снова вернул `124` уже без `-W`. Existing evidence не называл конкретную post-boot ADB-команду, поэтому новый trace-only candidate сохраняет command/exit в exact attempt log и freeze-ит log до cleanup. Static RED, hanging-ADB/`-W` fixtures и Android regression PASS 71.7 s, `All` PASS 182 negatives за 79.4 s. Fresh Tests сначала PASS 830/830 + 36/36, затем упал в отдельном `Toolbar_EmojiFilters…` 1/830; оба result не связаны с Android diff и классифицируются как suite flake до повторения. NuGet prerequisite остаётся `NEEDS-FIX`; final reset/native/merge и stages 4-10 pending
...
- Decision: stages 1-2 завершены. Stage 3 продолжает только approved harness/evidence/README truthfulness work; `Program.cs` first-run fix передан Stage 5. `libpulse0` и AVD-root barriers сняты, но replacement доказал independent readiness/teardown defect: `sys.boot_completed=1` — единственный gating signal, `bootanim` остаётся diagnostic; каждый ADB/AVD-manager call и процесс emulator имеют bounded cleanup. Повторить review/Distribution и isolated Tests rerun. Final acceptance всё равно сбрасывается после dependency-security prerequisite. В `rxs1` сначала закрыть все current review findings и получить повторные Role-Based/Post-SPEC PASS; затем отдельное точное approval, spec commit/implementation PR/merge и PR #280 rebase/full reset. Stage 4 закрыт до Stage-3 merge
- Review passes:
  ...
  - Android host-runtime ABI remediation: TDD runner-label RED 1.6 s и diagnostic-probe RED 1.9 s; `ubuntu-22.04` + diagnostic-preserving probe дали Bash syntax/Android regression PASS за 46.8 s и `All` PASS с 182 negatives за 108.8 s; independent review PASS с targeted 50.2 s и `All` 182/100.9 s; replacement CI pending.
  - Android libpulse runtime remediation: exact diagnostic из обоих API jobs = missing `libpulse.so.0`; host-step TDD RED 2.3 s, per-job bounded APT/package/SONAME assertions и negative fixture дали Android regression PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent review PASS с Android 51.8 s и `All` 182/91.4 s; replacement pending.
  - Android AVD-root isolation remediation: run `29838807057` подтвердил `libpulse0`/image/candidate в API23/API36, но `emulator` не увидел AVD, созданный `avdmanager`; unique exported root, no-HOME-fallback, descriptor/directory guards и EXIT cleanup дали static RED 4.4 s + negative RED 1.5 s, latest Bash + Android PASS 40.8 s и `All` 182/70.7 s; review/replacement pending.
  - Android readiness/ADB-boundedness remediation: run `30003714447` на PR candidate `ffd5ae6` доказал, что AVD isolation работает (API 36 launches booted twice), а `30006652106` доказал `adb_state=device`/`sys.boot_completed=1` и exact APK install в API23/API36. Strict `bootanim=stopped` и unbounded cleanup закрыты: final fixture принимает `sys.boot_completed=1` при `bootanim=running`, сохраняет ADB diagnostics и завершает hanging-ADB two-attempt failure with evidence within 12 s. Последний remaining timeout — post-boot `am start -W`; narrow RED/Green fixture удаляет `-W`, сохраняя PID/logcat gate. Bash + Android PASS 71.2 s, `All` 182/85.0 s; replacement/review pending.
  - Android per-command diagnostic remediation: `1156eb7` дал API 23 launch PASS, но API 36 exit `124` после no-`-W` request; preserved evidence содержал readiness, но не имя terminal ADB command. Wrapper теперь записывает command и status в content-addressed attempt log; hanging-ADB fixture требует `exit=124` + exact argv, а finalization flag запрещает EXIT cleanup менять уже хэшированные bytes. Android PASS 71.7 s, `All` 182/79.4 s; replacement pending.
...
- Evidence inspected: historical matrices сохраняют diagnostic trace. Run `29838807057` на `d0c68dd5` подтвердил scope/contract, Windows, macOS x64/arm64 и Android dual-APK build PASS; Linux expected `NU3012`, CodeQL `29838806826` и AndroidPkg `29838805889` PASS. Ubuntu 22.04 API23/API36 успешно установили/проверили `libpulse0`, image и exact APK, но preserved logs обоих jobs назвали `Unknown AVD name`. Run `30003714447` на `ffd5ae6` связал create/launch; `b1ebf24` run `30006652106` добавил exact readiness observations и изолировал `am start -W`; `1156eb7` run `30007920350` подтвердил full API 23 launch, однако API 36 вышел `124` после no-`-W` request. Новый candidate добавляет exact ADB argv/exit trace и freeze before EXIT cleanup, чтобы следующий API36 artifact различал runtime command failure от teardown. Static RED, hanging-ADB/`-W` fixtures, Android regression PASS 71.7 s и `All` PASS 182/79.4 s. Tests `30006652135` PASS Unit 830/830 + Headless 36/36; `30007920285` one-off `Toolbar_EmojiFilters_AllItemTogglesEveryEmojiFilter` 1/830 does not overlap current scope. Review/replacement CI и full exact-SHA reset pending
...
| MEDIUM | stage 3 / Android emulator command PATH | Run `29831446698` подтвердил readelf fix и built both APKs, но API23/API36 не нашли `emulator` после успешного sdkmanager install | Проверить exact executable, экспортировать `${ANDROID_SDK_ROOT}/emulator` в `$GITHUB_PATH` обоих jobs и закрепить missing-export regression | fixed in `0417ee22`; RED/GREEN/review PASS, run `29834142027` прошёл этот barrier и запустил emulator version probe |
| MEDIUM | stage 3 / Android emulator host ABI | Run `29834142027` в обоих API jobs завершил найденный emulator с `127`; command substitution скрыл loader stderr. Separate ELF scan: official 36.4.9/36.6.11/37.2.1 Linux packages требуют `libtiff.so.5`, Ubuntu 24.04 имеет только `libtiff6` | Использовать supported `ubuntu-22.04` для emulator-only jobs, закрепить per-job label и diagnostic-preserving version probe; не смешивать Jammy library с Noble | fixed in `1ef54245`; local/review PASS, run `29836756606` прошёл прежний opaque barrier и назвал следующий missing SONAME |
| MEDIUM | stage 3 / Android emulator libpulse runtime | Run `29836756606` в API23/API36 точно сообщил missing `libpulse.so.0` из qemu loader на Ubuntu 22.04 | Установить native `libpulse0` без recommendations через bounded APT retry; проверить package state и `/usr/lib/x86_64-linux-gnu/libpulse.so.0` в каждом job | fixed in `d0c68dd5`; run `29838807057` прошёл package/SONAME/image barriers и назвал следующий AVD-root defect |
| MEDIUM | stage 3 / Android emulator AVD-root isolation | Run `29838807057`: оба `avdmanager create` завершились, но `emulator` не увидел одноимённый AVD; host runtime/image/exact APK already PASS | Экспортировать unique temporary `ANDROID_AVD_HOME`, запретить shared HOME fallback, проверить exact `.ini` и `.avd` before launch, cleanup root только на EXIT и добавить missing-export regression | fixed in `ffd5ae6`; run `30003714447` прошёл этот барьер и достиг реального boot |
| HIGH | stage 3 / Android readiness and teardown | Run `30003714447`: API 36 дважды внутренне booted, но silent gate требовал optional `init.svc.bootanim=stopped`; API 23 остался в ADB/cleanup path дольше boot deadline | Gate только `sys.boot_completed=1`; писать ADB diagnostics в attempt log; bounded timeout для всех runtime ADB/`avdmanager` commands и escalation emulator teardown; добавить hanging-ADB + `bootanim=running` fixtures | fixed in `b1ebf24`; `30006652106` proved both APIs reached ready/install |
| HIGH | stage 3 / Android post-boot launch handoff | Run `30006652106`: API23/API36 reached ready/install, then `am start -W` exhausted the new 30-s command timeout | Request nonblocking activity start; retain existing post-launch wait, live-process and fatal-logcat gates; fixture must hang only old `-W` call | fixed locally; Android regression 71.2 s и `All` 182/85.0 s PASS; replacement pending |
| HIGH | stage 3 / Android API 36 terminal command | Run `30007920350`: API 23 launch PASS after no-`-W`, API 36 still exit `124`; existing evidence lacked terminal ADB argv/status | Record every bounded ADB command/result in content-addressed attempt log and prohibit post-finalization cleanup mutation; reproduce timeout fixture | fixed locally; Android regression 71.7 s и `All` 182/79.4 s PASS; replacement pending |
| MEDIUM | CI / unrelated UI suite | Companion Tests `30003714491` дал 12 failures `MainControlTaskCardLayoutUiTests` (818/830), тогда как текущий diff меняет только Android harness/tests/spec | Rerun isolated Tests job; только повторяемый failure эскалировать в отдельный UI scope/spec | closed as non-reproducible: `30006652135` Unit 830/830, Headless 36/36 PASS |
...
- Checks rerun: run `29838807057` подтвердил libpulse runtime/image/candidate barriers в обоих API jobs, после чего оба fail-closed точно на unknown AVD name. `ffd5ae6` fixed AVD-root, `b1ebf24` fixed readiness/teardown; run `30006652106` прошёл scope/contract/Windows/macOS/Android build, Linux expected `NU3012`, оба API boot/install и точно выявил only post-boot `am start -W` wait. RED/Green nonblocking launch fixture, Bash + Android PASS 71.2 s и `All` 182/85.0 s. Tests rerun `30006652135` PASS 830/830 + 36/36. Review и replacement Distribution pending. Любой tracked commit сбрасывает прежний full Unit/Headless/native evidence; final exact-SHA reset ещё не выполнен
- Validation evidence: stage 1 — PASS/merged; Stage 2 — PASS/merged; Headless prerequisite — PASS/merged PR #279; Stage 3 — runs `29794200613`, `29821815902`, `29822419518`, `29824937816`, `29827483394`, `29829779408`, `29831446698`, `29834142027`, `29836756606`, `29838807057`, `30003714447` (cancelled after diagnostics), `30006652106` (post-boot launch timeout). CodeQL `30006652065`, AndroidPkg `30006652062`, Tests `30006652135` Unit 830/830 + Headless 36/36 PASS. Nonblocking-launch fix Bash + Android + `All` 182 PASS; review/commit/replacement pending. `S3-AC-20` ждёт NuGet prerequisite, clean-archive/full-test/native evidence; final aggregate/review/merge pending; stages 4-10 не стартованы
...
| EXEC | Исправить Android emulator host ABI | 0.99 | Commit/push и replacement CI | Доставить independently reviewed `ubuntu-22.04` для двух emulator jobs и diagnostic-preserving version helper | Нет | Не применимо | Separate official 36.4.9/36.6.11/37.2.1 ELF scan требует `libtiff.so.5`; runner/probe RED 1.6/1.9 s, local + independent targeted/`All` и review PASS | Distribution workflow/Android validator/regression, Stage-3 child spec, roadmap |
...
| EXEC | Исправить Android emulator libpulse runtime | 0.99 | Commit/push и replacement CI | Доставить per-job bounded `libpulse0` install + package/SONAME guards | Нет | Не применимо | TDD RED 2.3 s; Android 43.5 s + `All` 182/103.8 s; independent Android 51.8 s + `All` 182/91.4 s, review PASS | Distribution workflow/Android regression, Stage-3 child spec, roadmap |
| EXEC | Выполнить replacement matrix на `d0c68dd5` | 1.00 | Android emulator AVD-root isolation fix | Получить exact failure evidence и связать producer/consumer AVD home | Нет | Не применимо | Run `29838807057`: scope/contract/Windows/macOS/Android build PASS, Linux expected `NU3012`; API23/API36 installed libpulse/image/candidate PASS, затем unknown AVD name | PR #280, exact-head Actions runs |
| EXEC | Исправить Android emulator AVD-root isolation | 0.99 | Review, commit/push и replacement CI | Доставить unique exported `ANDROID_AVD_HOME`, no-HOME-fallback, exact descriptor/directory guards и EXIT cleanup | Нет | Не применимо | Static RED 4.4 s + negative RED 1.5 s; latest Bash + Android 40.8 s; `All` 182/70.7 s PASS | Distribution workflow/Android regression, Stage-3 child spec, roadmap |
| EXEC | Диагностировать replacement matrix на `ffd5ae6` | 1.00 | Harness readiness/teardown fix | Извлечь preserved API evidence, не повышать support | Нет | Не применимо | Run `30003714447`: AVD-root barrier снят; API 36 дважды booted 54.9/56.9 s, затем strict bootanim gate дал failure; API 23 remained in launch step beyond deadline, run cancelled | PR #280, failure artifact, exact workflow run |
| EXEC | Исправить Android readiness и bounded teardown | 0.99 | Independent review и replacement CI | Gate `sys.boot_completed=1`, log ADB observations, bound ADB/AVD manager and emulator teardown | Нет | Не применимо | RED fixtures; Android regression 63.4 s и `All` 182/77.9 s PASS. Tests companion `30003714491` red 818/830 pending isolated rerun | Android validator/regression, Stage-3 child spec, roadmap |
| EXEC | Диагностировать readiness replacement matrix на `b1ebf24` | 1.00 | Post-boot launch handoff fix | Preserve exact API evidence; do not treat boot/install as launch verification | Нет | Не применимо | Run `30006652106`: desktop/Android build PASS, Linux expected `NU3012`; both API cells reached ready/install, then `am start -W` timed out. Tests rerun `30006652135` PASS Unit 830/830 + Headless 36/36 | PR #280, exact workflow run and failure artifacts |
| EXEC | Исправить Android post-boot launch handoff | 0.99 | Review, commit/push и replacement CI | Remove only blocking `-W`; retain wait/PID/logcat launch proof and fixture | Нет | Не применимо | Static RED, hanging-`-W` dynamic success; Bash + Android 71.2 s и `All` 182/85.0 s PASS | Android validator/regression, Stage-3 child spec, roadmap |
| EXEC | Диагностировать API 36 terminal timeout на `1156eb7` | 1.00 | Per-command trace | Не выдавать API23 PASS за API36 support; сохранить exact artifact | Нет | Не применимо | Run `30007920350`: API23 full launch PASS; API36 exit 124 after no-`-W`; Tests result alternates unrelated flaky failures | PR #280, exact workflow run and artifacts |
| EXEC | Добавить content-addressed ADB terminal trace | 0.99 | Review, commit/push и replacement CI | Log command/exit before freeze; no cleanup mutation after evidence finalization | Нет | Не применимо | Static RED; hanging-ADB status/argv fixture; Android 71.7 s, `All` 182/79.4 s PASS | Android validator/regression, Stage-3 child spec, roadmap |
...
| EXEC | Закрыть review finding для install-timeout evidence/isolation | 0.99 | Replacement native evidence и final green CI | Требовать bounded install failure evidence и возврат общего timeout после успешного install/во время failure cleanup | Нет | Не применимо | Adversarial review нашёл, что initial fixture не доказывал timeout upper bound и отсутствие leakage. New RED выявил реальный wrapper edge case: trace `exit=124` был, но ERR-handler не записывал `evidence.json`; follow-up review выявил, что local install override продлевал cleanup. Override теперь охватывает только install call, затем `handle_emulator_error` работает с general bound. Fixtures требуют delayed-install success, install `124` + evidence, cleanup `emu kill` `124` и post-install `force-stop` `124` при general=1/install=3. Bash PASS; Android regression PASS 94.1 s; `All` 182 negatives PASS 65.1 s | Android validator/regression, review finding, Stage-3 child spec, roadmap |
| EXEC | Сохранить API 36 runtime logcat до PID failure | 0.99 | Replacement native evidence, затем diagnosis process exit | Capture logcat после launch wait, до fatal/PID guards; не менять application/runtime behaviour | Нет | Не применимо | Run `30010971884` (first install-timeout candidate) подтвердил API 36 `install`, `force-stop` и `am start` exit 0, но процесс отсутствовал; прежний failure artifact не содержал logcat, потому что capture был после PID guard. New TDD RED; missing-process fixture требует non-empty `android-api23-logcat.txt` в failure evidence. Bash PASS; Android regression PASS 116.4 s; `All` 182 negatives PASS 65.7 s | PR #280, artifact `distribution-android-api36-a1a6d18c58bb-failure-attempt-1`, Android validator/regression |
| EXEC | Подтвердить Android launch evidence после install/logcat remediation | 1.00 | Отдельно диагностировать NuGet и TUnit blockers | Не менять product/runtime code; подтвердить exact x64 launch на обеих declared API | Нет | Не применимо | Run `30013347931` для PR head `1959ad8` (merge source/workflow `b75813d`) дал `android_api23=success` и `android_api36=success`. API 36 evidence: `processId=3776`, `fatalLogcatEntries=0`, `supportLevel=launchVerified`; artifact содержит emulator logcat 5,312,820 bytes. Machine verdict падает только на `linux_x64=NU3012`. Companion Tests `30013347823` attempt 1 и rerun attempt 2 оба cancelled ровно по `timeout-minutes: 30`; логи доходят до `Running tests from ... Unlimotion.Test.dll`, но не называют test result. Timeout повторяем, но evidence не связывает его с четырьмя Android/spec файлами этого diff; отдельная диагностика нужна до изменения test workflow | PR #280; `distribution-android-api36-b75813d12ad5-attempt-1`; `distribution-verdict-b75813d12ad5-attempt-1`; Tests `30013347823` |
