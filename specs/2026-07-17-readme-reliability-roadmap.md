...
### Post-EXEC Review
- Статус: `NEEDS-FIX`; stages 1-2 и Headless prerequisite PR #279 доставлены, draft Stage-3 PR #280 открыт. Commit `d0c68dd5` добавил native Jammy `libpulse0`: run `29838807057` дал contract/Windows/macOS/Android dual-APK build PASS и expected Linux `NU3012`; companion Tests `29838805611`, CodeQL и AndroidPkg PASS. Оба API jobs успешно прошли host runtime/image/exact APK, затем `emulator` одинаково сообщил unknown AVD name. Новый validator экспортирует unique runner-temporary `ANDROID_AVD_HOME`, запрещает fallback к `$HOME/.android/avd`, проверяет descriptor/directory перед launch и удаляет isolated root в EXIT cleanup; static RED 4.4 s + shared-HOME negative RED 1.5 s, latest Bash + Android PASS 40.8 s и `All` PASS с 182 negatives за 70.7 s. NuGet prerequisite остаётся `NEEDS-FIX`; final reset/native/merge и stages 4-10 pending
...
- Decision: stages 1-2 завершены. Stage 3 продолжает только approved harness/evidence/README truthfulness work; `Program.cs` first-run fix передан Stage 5. `libpulse0` снял host ABI barrier; replacement точно показал разрыв producer/consumer AVD home. Экспортировать unique temporary `ANDROID_AVD_HOME`, fail-closed проверить exact `.ini`/`.avd` до launch и удалить root в EXIT cleanup; повторить review/Distribution/Tests. Final acceptance всё равно сбрасывается после dependency-security prerequisite. В `rxs1` сначала закрыть все current review findings и получить повторные Role-Based/Post-SPEC PASS; затем отдельное точное approval, spec commit/implementation PR/merge и PR #280 rebase/full reset. Stage 4 закрыт до Stage-3 merge
- Review passes:
...
- Evidence inspected: historical matrices сохраняют diagnostic trace. Run `29838807057` на `d0c68dd5` подтвердил scope/contract, Windows, macOS x64/arm64 и Android dual-APK build PASS; Linux expected `NU3012`, CodeQL `29838806826` и AndroidPkg `29838805889` PASS. Ubuntu 22.04 API23/API36 успешно установили/проверили `libpulse0`, image и exact APK, но preserved logs обоих jobs назвали `Unknown AVD name`: `avdmanager create` не был связан с поиском `emulator`. Validator создаёт unique root через `mktemp`, экспортирует `ANDROID_AVD_HOME`, запрещает shared HOME fallback, доказывает exact `.ini`/`.avd` before launch и удаляет root via EXIT cleanup; static RED 4.4 s + shared-HOME negative RED 1.5 s, latest Bash + Android regression PASS 40.8 s, `All` PASS с 182 negatives за 70.7 s. Review/replacement CI и full exact-SHA reset pending
...
- Checks rerun: run `29838807057` подтвердил libpulse runtime/image/candidate barriers в обоих API jobs, после чего оба fail-closed точно на unknown AVD name. AVD-root regression дал expected static RED 4.4 s и shared-HOME negative RED 1.5 s; latest Bash + Android PASS 40.8 s и `All` 182/70.7 s. Review и replacement Distribution pending. Любой tracked commit сбрасывает прежний full Unit/Headless/native evidence; final exact-SHA reset ещё не выполнен
- Validation evidence: stage 1 — PASS/merged; Stage 2 — PASS/merged; Headless prerequisite — PASS/merged PR #279; Stage 3 — runs `29794200613`, `29821815902`, `29822419518`, `29824937816`, `29827483394`, `29829779408`, `29831446698`, `29834142027`, `29836756606`, `29838807057`. Latest companions: Tests `29838805611`, CodeQL `29838806826` и AndroidPkg `29838805889` PASS. AVD-root fix static + Android + `All` 182 PASS; review/commit/replacement pending. `S3-AC-20` ждёт NuGet prerequisite, clean-archive/full-test/native evidence; final aggregate/review/merge pending; stages 4-10 pending
