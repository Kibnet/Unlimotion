...
### Post-EXEC Review
- Статус: `NEEDS-FIX` (review in progress).
- Scope reviewed: approved spec, exact implementation diff, focused TUnit contract, local `Full` receipt and the current-candidate GitHub Actions checks.
- Finding HIGH: initial `android-release` checked only the number of ZIP entries and that at least one entry matched an APK pattern, then published `*.apk` with `--clobber`. Thus a digest-valid artifact containing one expected APK and one unexpected APK could cross the write-authority boundary and overwrite an existing release asset. This contradicts the approved closed-manifest, explicit-path and collision-stop contract.
- Required correction: build an attempt-qualified artifact with an exact two-APK manifest; verify API metadata, exact ZIP entry set, regular extracted files and manifest hashes/lengths; pass only the two explicit validated paths to a pinned release action with `overwrite_files: false`. Re-run the focused contract and exact candidate CI before a final Post-EXEC verdict.
- Decision: protected-merge action remains prohibited until this correction and the re-review are `PASS`.
