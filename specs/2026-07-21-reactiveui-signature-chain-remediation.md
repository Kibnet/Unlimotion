...
| EXEC | Regression zero/null native-exit binding correction | 1.00 | fresh exact-SHA Windows Regression CI | preserve `0` as a valid native exit and null only for unattempted runs | Нет | Нет | CI run `30048112318`, Regression job `89343907496`, completed the long Unit invocation but then rejected the producer before evidence creation: PowerShell had unboxed a non-null `Nullable<Int32>`, so `HasValue` was invalid. The record builder now accepts object-or-null values and explicitly casts only non-null inputs. SelfTest covers both native exit `0` and a null not-attempted run, and also validates the synthetic `test-evidence-failed` primary receipt through the independent validator. This row does not claim a successful rerun. | scripts/Test-NuGetSignatureChain.ps1, GitHub Actions run `30048112318`, local SelfTest, local TUnit report, this spec |
