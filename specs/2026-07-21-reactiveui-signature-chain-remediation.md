...
Каждый test invocation использует `dotnet run --project <exact-absolute-project> -c Debug --no-restore --no-build -- --maximum-parallel-tests 1 --report-trx --report-trx-filename results.trx --report-html-filename <absolute-unique-raw-run-root>/results.html --results-directory <absolute-unique-raw-run-root>`. Runner args идут strictly after `--`; `--report-html` не передаётся, поскольку TUnit `1.44.0` генерирует HTML по умолчанию. Pinned TRX extension rejects a directory component in `--report-trx-filename`, so its value is exact bare `results.trx` and absolute `--results-directory` owns placement; HTML filename is absolute. Each root is distinct and absent before orchestrator creation. Its acceptance payload is exact one top-level regular `results.trx` plus one top-level regular `results.html`; TUnit additionally creates exactly one non-link implementation-owned shadow `<runner-id>/In/<machine-id>/results.html`, and adapter accepts it only when all three path components are ASCII `[A-Za-z0-9._-]+`, it is the sole recursive sidecar and its bytes are identical to top-level `results.html`. Any other sidecar, attachment, link, directory shape, byte difference or cwd report is `test-evidence-failed`; shadow is never parsed, sanitized or uploaded. Before child launch adapter sets supported exact `TUNIT_DISABLE_GITHUB_REPORTER=true` and removes all `GITHUB_OUTPUT`, `GITHUB_ENV`, `GITHUB_PATH`, `GITHUB_STATE`, `GITHUB_STEP_SUMMARY`, `ACTIONS_RUNTIME_TOKEN`, `ACTIONS_RESULTS_URL`; unsupported invented TUnit toggles are not contract. This forbids GitHub reporter/HTML upload/summary side effects; adapter proves exact root contents and no created report/summary outside root. Unit and Headless branches remain independent after prerequisites. `Invoke-TestCommandAdapter` only runs native process, retains raw reports outside candidate and parses exact named files into typed counts/paths; it never creates sanitized reports. Sanitizer regenerates closed minimal TRX/HTML under `candidateEvidenceRoot/regression` and validates them. Complete receipt records exact SHA/lane/projection, native vs synthetic exit, counts and regenerated report hashes.
...
### Post-EXEC Review
- Статус: `PASS`.
- Scope reviewed: approved spec, exact implementation diff `e11cae9…2e505492`, focused TUnit contract, local `Full` receipt, independent `SelfTest` and exact-candidate GitHub Actions checks.
- Fixed HIGH: initial `android-release` checked only the number of ZIP entries and that at least one entry matched an APK pattern, then published `*.apk` with `--clobber`. Commit `70aa9a8` replaces it with an attempt-qualified closed two-APK manifest, metadata/API and ZIP-entry binding, regular-file/hash/length validation and two explicit paths to pinned `softprops/action-gh-release` with `overwrite_files: false`. Candidate `2e505492747b6b276089b064e7b4d4a291eb5641` passed `android-build`, so the new build-side manifest/upload contract is executed on GitHub; the release-only job is correctly skipped for a PR.
- Regression re-review: commit `2e50549` preserves only a validated aggregate TRX summary when a native test command is nonzero; raw reports, process streams, test names, exception text and secrets remain unpublished and ephemeral. `SelfTest` and the focused executable contract pass locally; the exact-candidate Windows `Regression` producer, independent validator, artifact upload and verdict enforcement all passed.
- Debian boundary: this security spec authorizes its fresh-signature-restore, pinned-action and read-build/write-release split, plus archive digest verification. Its pre-existing release-asset replacement semantics are not the Android closed-manifest contract and are deliberately not extended here; a Debian release-asset manifest/collision redesign belongs to the separately frozen Stage-4 publication scope.
- External re-review: `Signature`, `Regression`, `android-build`, CodeQL actions/C#/JavaScript analysis and CodeQL all passed for the exact candidate. `Upload Android release assets` is an expected `skipped` release-only check on a pull request.
- Decision: technical Post-EXEC gate is closed. Protected-merge configuration on `main`, draft-to-ready transition and merge remain a separate explicit human decision; no settings or PR state were changed by this review.
...
| EXEC | candidate full-graph guard | 0.98 | closed worker/finalizer implementation | preserve the guard while moving verification into the framed worker | Нет | Нет | Signature restores now stage each project assets file immediately, compare every normalized id/version/SHA-512 graph against the immutable parent fixture, and allow only the six approved ReactiveUI/Splat transitions. Synthetic SelfTest rejects unrelated drift. A fresh isolated Signature attempt on the candidate completed 13 phases: three staged assets, one graph guard and six fingerprint-bound package verifications; the independent receipt validator accepted it. The first real attempt exposed a project-assets staging path defect; it was corrected before the successful rerun. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, temp evidence unlimotion-graph-guard-retry-dc8245ee2ac1492a80d2524a0ff21673, this spec |
| EXEC | closed worker protocol foundation | 0.94 | external worker adapter, sanitizers and finalizer | route SignatureVerify through an isolated worker process, then add the remaining closed kinds | Нет | Нет | Added a one-frame, 4-byte-length-prefixed stdin protocol with a 1 MiB bound, exact JSON schema, ordinal secret-seed-name hash and no CLI binder path for kind/payload/seeds. Worker pipe characterization proved an incomplete SignatureVerify payload returns only a bounded non-secret failure frame with nonzero exit and empty stderr; the seed value was absent from the response. SignatureVerify handler now validates the exact three absolute assets paths, parent graph and six fingerprint-bound packages, but RunAttempt still invokes its legacy local verification until the external process adapter is complete. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local pipe test, local TUnit report, this spec |
| EXEC | framed nested-payload correction | 1.00 | isolate external adapter lifecycle separately | keep RunAttempt on the already-validated local verification path until the adapter has its own passing lifecycle fixtures | Нет | Нет | Attempting to connect the external adapter uncovered that pipeline serialization enumerated an ordered dictionary, producing a malformed multi-value input frame. The serializer now uses ConvertTo-Json InputObject and SelfTest round-trips a nested three-assets payload. The adapter hookup was deliberately removed rather than leaving Signature attempts broken; repeated fresh restore was additionally affected by NuGet EOF transport retries, so it is not counted as worker success evidence. | scripts/Test-NuGetSignatureChain.ps1, local SelfTest, this spec |
| EXEC | framed worker length correction | 0.99 | reconnect the external adapter with bounded lifecycle fixtures | keep Signature on the validated in-process verifier until adapter integration has its own evidence | Нет | Нет | A real child process with a 695-byte request revealed that the decoder shifted `[byte]` values directly: `0x02 << 8` overflowed to zero and made a valid frame appear to have length 179. The decoder now promotes each header byte to `Int32`; SelfTest covers a frame whose length exceeds 255, the static contract anchors that guard, and an external child negative probe returned the closed non-secret failure frame with empty stderr. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local worker pipe probe, this spec |
| EXEC | external Signature worker adapter | 0.97 | sanitizer/finalizer workers and full lifecycle hardening | keep only the verified Signature worker path connected; no completion claim for unimplemented worker kinds | Нет | Нет | Added a no-profile external `pwsh` adapter with canonical framed stdin, concurrent bounded stdout/stderr draining, timeout/overflow termination tuple and typed closed-result validation. Signature now receives graph and all six package receipts only from `WorkerKind=SignatureVerify`. A fresh isolated attempt `701` completed all 13 phases; its independent receipt validator accepted the output. The external command wrapper timed out before it could collect the already-complete local process, but the receipt and process inspection prove the worker itself completed normally. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, temp attempt `unlimotion-nuget-evidence-83d47050…-attempt-701-*`, local `Test-NuGetEvidencePublication.ps1`, this spec |
| EXEC | closed secret-seed snapshot | 0.96 | sanitizer encoding scanner and sibling validator seed binding | keep seed values in process memory and framed worker input only | Нет | Нет | Added deterministic detection of closed secret environment names, excludes explicit non-seeds, enforces the 64-entry and UTF-8-size bounds, supplies the Signature worker with that snapshot and removes every snapshotted name from its environment. SelfTest proves only API_TOKEN/ConnectionString are seeded, and the child returns the name hash without value disclosure. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Signature candidate sanitizer foundation | 0.94 | integrate phase-derived production payload and publication finalizer | candidate root remains outside the final/upload root until full manifest validation | Нет | Нет | Implemented `SignatureSanitize` as a closed worker: it requires an absent candidate root, emits only deterministic per-package logs plus `signature/evidence.json`, validates relative output containment and re-scans every emitted byte sequence for NULs, absolute worker paths and framed secret values. SelfTest uses an external worker with two synthetic seeds and proves the exact seven-file candidate set contains neither value. It is deliberately not yet wired to RunAttempt because the phase-dependent failure projection/finalizer are still incomplete. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Publication finalizer foundation | 0.94 | phase-dependent primary/fallback projection and independent final-tree validator | retain candidate isolation until the complete finalizer boundary is integrated | Нет | Нет | Added the closed `PublicationFinalize` worker for the success-shaped candidate path. It validates a non-link candidate manifest with ordinal/casefold uniqueness and sanitized bytes, writes a primary receipt whose manifest excludes itself, revalidates the scratch tree and atomically publishes only after the eight-file SelfTest tree is complete. The diagnostic stderr probe used to locate a PowerShell predicate parsing defect was removed; child errors again collapse to `worker-failed`. It is not wired to `RunAttempt` and does not yet claim fallback/error precedence. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Publication fallback foundation | 0.93 | phase-dependent failure projection, native same-volume identity and independent final-tree validator | retain unintegrated candidate isolation and do not claim catastrophic/post-return handling | Нет | Нет | The closed finalizer now treats invalid candidate/scratch publication as a recoverable publication-integrity defect: it removes scratch and publishes only `attempt-receipt.json` with the strict `safe-fallback` shape. SelfTest proves both the eight-file primary tree and a missing-candidate one-file fallback tree. Directory links and unexpected primary paths are rejected before publication. It does not yet distinguish catastrophic root failures, expose phase-derived receipt failure identity or call the finalizer from `RunAttempt`. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Primary outcome guard | 0.95 | strict phase-failure projection into signature-failure evidence | keep failed-phase evidence fallback-only until that projection is implemented | Нет | Нет | `PublicationFinalize` now validates each input phase tuple and refuses to publish a success-shaped primary tree if any recorded phase failed. A valid candidate plus a failed phase therefore produces the one-file safe fallback, which SelfTest covers separately from a missing candidate. Primary receipts explicitly carry null `failurePhase` and `failureCode`; phase-dependent failure primary remains intentionally unimplemented. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Scratch manifest byte binding | 0.96 | native same-volume identity and post-rename tamper gate | validate scratch bytes before publication without promising recovery after a published-tree mutation | Нет | Нет | The finalizer now constructs a receipt hash entry and compares the complete scratch manifest path/hash/length tuple against the pre-copy candidate manifest before rename; matching only the file count or names is no longer sufficient. SelfTest has a synthetic same-path hash-mutation negative. This is a pre-rename guard only: native file identity, link-count and post-return independent validation are still pending. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Signature failure candidate sanitizer | 0.93 | integrate legal phase projection and primary failure publication | retain fallback-only finalizer behaviour for failed phases until all tuples are strict | Нет | Нет | Added a closed `SignatureSanitize` failure payload shape that emits `signature-failure` rather than reusing success-only graph/package fields. It preserves only completed-project and attempted-package prefixes, writes logs only for attempted packages, and removes the entire candidate on any sanitization failure. SelfTest proves the two-file one-package failure candidate has no success-only `projects`/`packages` fields. This worker is not yet invoked from `RunAttempt`, and `PublicationFinalize` still routes a failed phase to safe fallback pending the strict four-field phase/receipt projection. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Signature failure primary publication | 0.94 | integrate canonical phase producer and independent validator | keep mismatched candidate/phase combinations fallback-only | Нет | Нет | `PublicationFinalize` now accepts closed four-field phase tuples and derives primary receipt outcome, `failurePhase` and `failureCode` from the first failed phase. It requires `signature-success` when all phases succeeded and `signature-failure` otherwise; mismatch remains a safe fallback. SelfTest publishes a three-file failure primary tree and verifies the projected receipt. `RunAttempt` still produces legacy three-field phases, so this capability is intentionally not connected until its canonical phase runner and validator update land together. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Signature producer-to-validator transition | 0.96 | canonical failure-path runner and Regression/Full migration | retain legacy Regression receipt only as an explicit temporary compatibility path | Нет | Нет | Successful Signature `RunAttempt` now sanitizes a fresh candidate, publishes the closed primary tree and emits four-field receipt phases. The validator detects `attempt-receipt.json`, verifies recursive manifest path/hash/length closure and Signature evidence-kind/outcome consistency; it still recognizes the old top-level `attempt.json` for unchanged Regression. Real isolated attempt `703` on `07fb178…` produced 8 files and 14 receipt phases; independent validator PASS. Attempt `702` exposed and corrected two defects before acceptance: path normalization used the wrong literal replacement, and `JsonElement.Clone()` serialized receipt phases as `ValueKind` objects. Desktop Debug restore was re-established afterwards. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, temp attempt `…-702-*` rejected, `…-703-*` accepted, local validator, this spec |
| EXEC | Signature CI worker executable resolution | 0.99 | fresh exact-SHA Signature result | select the resolved `dotnet` command instead of treating PATH aliases as conflicting executables | Нет | Нет | Candidate Signature job `89328270942` failed in `SignatureSanitize`: `Get-Command dotnet -All` found multiple valid PATH entries and incorrectly rejected the worker before sanitization. The worker now uses PowerShell's selected command and still requires a fully-qualified existing executable. The subsequent legacy failure-receipt validation message is a cascade from that early failure and remains a separate failure-path follow-up; this row does not claim a green rerun. | scripts/Test-NuGetSignatureChain.ps1, GitHub Actions job `89328270942`, this spec |
| EXEC | Signature CI adapter executable resolution | 0.96 | fresh exact-SHA Signature result | apply the selected-command resolution to `pwsh` and retain the absolute-path and WindowsApps checks | Нет | Нет | After the `dotnet` repair, Signature job `89329927039` restored all three projects and safely uploaded a failure receipt, but the closed Signature adapter returned `native-command-threw` before a worker response. The remaining `Get-Command pwsh -All` check has the same multiple-PATH-entry failure mode on Ubuntu (typically symlink plus target), so it is replaced with PowerShell's selected command. This is evidence-based diagnosis, not a claim of remote confirmation; `SelfTest` and the static CI contract pass locally and a new candidate run is required. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, GitHub Actions job `89329927039`, this spec |
| EXEC | Signature canonical failure publication | 0.97 | fresh exact-SHA failure-path CI and Regression migration | route every recoverable Signature attempt failure through the existing closed failure sanitizer and finalizer | Нет | Нет | The last legacy `attempt.json` writer in the Signature catch branch is removed. The attempt now canonicalizes phase tuples, preserves the first recorded failure code, creates phase-appropriate `signature-failure` candidate evidence and uses `PublicationFinalize`; sanitizer failure itself causes the finalizer's one-file safe fallback rather than a mutable legacy receipt. An integration SelfTest proves a `native-command-threw` worker failure yields a primary closed receipt with that phase/code, and the static CI contract passed 1/1. Precondition bootstrap and Regression still retain their separate unfinished migration work. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Regression sanitizer/publication foundation | 0.93 | raw TRX/HTML adapter, strict report projection and RunAttempt migration | implement the closed `RegressionSanitize` worker and make finalizer/validator lane-aware without enabling it in the legacy Regression orchestrator | Нет | Нет | `RegressionSanitize` now accepts only a closed three-run summary and writes `regression/evidence.json` outside the final root. `PublicationFinalize` and the independent validator derive the exact `signature|regression` evidence prefix from the lane and accept neither cross-lane tree. SelfTest creates a Regression candidate, publishes it and invokes the independent validator successfully. This deliberately does not claim raw report redaction/count validation or production Regression migration. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Independent primary phase binding | 0.98 | complete phase graph/schema and legacy Regression removal | verify every primary receipt phase and bind failure identity to the first failing tuple | Нет | Нет | The independent gate now requires exact `{name,status,exitCode,failureCode}` phase objects, lane-prefixed phase names and valid success/failure exit tuples. It rejects a success receipt with any failed phase and requires a failure receipt to cross-link `failurePhase` and `failureCode` to the first failed tuple. The self-test independently validates both new Signature failure evidence and Regression foundation evidence; the static contract passed 1/1. This remains narrower than the approved full ordered phase graph. | scripts/Test-NuGetEvidencePublication.ps1, scripts/Test-NuGetSignatureChain.ps1, local SelfTest, local TUnit report, this spec |
| EXEC | Regression raw-report adapter and production receipt migration | 0.95 | fresh exact-SHA Windows Regression CI and remaining deadline/identity fixtures | retain raw reports only in distinct temporary roots, publish regenerated evidence and remove legacy Regression receipt path | Нет | Нет | TUnit `1.44.0` was characterized against the Unit and Headless projects: it accepts bare `results.trx`, treats `--report-html` as deprecated and emits an implementation-owned duplicate HTML shadow below `<runner>/In/<machine>/`. The adapter accepts only that exact non-link byte-identical shape, parses secure TRX counters, applies Unit/Headless floors and removes raw roots. `RegressionSanitize` now emits typed three-run evidence and regenerated minimal reports; RunAttempt invokes closed sanitizer/finalizer and the independent validator checks report references/hashes, canonical report bytes, count invariants and headless equality. `SelfTest` and `StormCiReadmeMediaExecutableSpecTests` passed. A complete local attempt was intentionally stopped only by Codex's outer 20-minute command limit while its sequential Unit child was still running; the resulting process tree was terminated and is not counted as validation. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Regression zero/null native-exit binding correction | 1.00 | fresh exact-SHA Windows Regression CI | preserve `0` as a valid native exit and null only for absent fields | Нет | Нет | CI run `30048112318`, Regression job `89343907496`, completed the long Unit invocation but then rejected the producer before evidence creation: PowerShell had unboxed a non-null `Nullable<Int32>`, so `HasValue` was invalid. The first rerun (`30048911702`, job `89346403603`) reached phase projection and exposed the companion binding issue: omitted `[string]` parameters materialized as `''`, not null. The record builder now accepts object-or-null numeric values and normalizes empty `failureCode`/`skipReason` strings to null. SelfTest covers zero, null, canonical successful phase and synthetic `test-evidence-failed` primary receipt through the independent validator. This row does not claim a successful rerun. | scripts/Test-NuGetSignatureChain.ps1, GitHub Actions runs `30048112318`, `30048911702`, local SelfTest, local TUnit report, this spec |
| EXEC | Regression exact-SHA CI confirmation | 1.00 | Full local wrapper and remaining hardening fixtures | retain the independently validated Signature/Regression receipts as the proven child-lane baseline | Нет | Нет | GitHub Actions run `30049835912` for source `c16b6c520b49bc179ecfe5b0bce522b79715d241` completed `success`: Signature job `89349310696` and Windows Regression job `89349310733` both passed their producer, independent validator, artifact upload and enforcement steps. Regression took 12m34s from start to successful validated evidence; this is the first remote confirmation after the zero/null correction, not evidence for the still-unimplemented Full outer contract or protected-merge configuration. | GitHub Actions run `30049835912`, jobs `89349310696` and `89349310733`, this spec |
| EXEC | Full outer receipt and recursive validator foundation | 0.96 | real committed-clean Full diagnostic, deadlines/identity fixtures and external delivery gate | keep Full local-only and non-authoritative until its real child envelopes are run on an immutable SHA | Нет | Нет | Added the `Full` local Windows wrapper with two separate child processes and isolated package roots. It accepts only independently validated primary child receipts, produces a `full-primary` outer receipt with exact ordered `{Signature/signature, Regression/regression}` cross-links and hashes every nested child file; child fallback/missing evidence becomes a one-file outer fallback. Child sanitizers now carry the explicit `full-child` runtime context, while the independent validator recursively validates both nested receipts and every outer manifest hash. `SelfTest` creates a synthetic Signature-failure plus Regression-success Full tree and validates it recursively; PowerShell parser checks and the rebuilt static TUnit contract pass. No real Full attempt was run here because it can consume up to 175 minutes and must use a committed clean source SHA. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Full child-work isolation correction | 0.97 | rerun a committed-clean real Full diagnostic, then implement deadline/identity fixtures | preserve only independently validated child final trees in the outer candidate; all raw/candidate child paths remain outside it | Нет | Нет | Real Full attempt `1` on committed clean `2bd38b10…` completed Signature and all Regression test children, but safely published only a one-file outer fallback. The structural cause was identified: Regression used an evidence root below the outer candidate, so its transient `*.raw` sibling could remain in the outer scanner’s scope when cleanup was delayed. Full now creates each child’s `packages`, `final`, `final.raw` and `final.candidate` below an absent child work root, validates the child’s final receipt there, then copies only that final tree into `candidate/signature` or `candidate/regression`. The SelfTest now independently validates both allowed Full-failure orders, including Signature success plus Regression failure under `full-child`; parser check passed. A new real Full result is not claimed until this correction is committed and rerun. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local Full attempt `1` outer fallback, local SelfTest, this spec |
| EXEC | Full source-checkout isolation | 0.98 | commit and rerun a clean Full diagnostic; then deadlines/identity/adversarial fixtures | preserve the caller checkout and move all restore/build/test writes into child-owned detached worktrees | Нет | Нет | The first Full run also left ignored `obj/project.assets.json` files in the caller checkout pointing to the deleted isolated NuGet cache; the subsequent no-restore build failed until a normal targeted restore rewrote those generated assets. Each child now creates a detached temporary Git worktree at the exact source SHA, runs all restore/build/test writes there, validates its final evidence, removes the worktree through Git, and only then exposes a byte-copy of the final tree to outer aggregation. Full additionally requires the caller checkout to be clean and its `HEAD` to equal the requested SHA before creating a child. `SelfTest` and PowerShell parser check pass; this row intentionally does not claim the required post-commit real Full result. | scripts/Test-NuGetSignatureChain.ps1, local targeted restore and build recovery, local SelfTest, this spec |
| EXEC | Full short-path source worktree | 0.98 | commit and rerun a clean Full diagnostic; then deadlines/identity/adversarial fixtures | avoid Windows long-path bootstrap failure without placing child raw/evidence data outside the controlled temp layout | Нет | Нет | The first detached-worktree rerun safely fell back before a child started: a `%TEMP%` worktree exceeded Windows filename limits for tracked `.qoder/repowiki` paths. A direct probe proved `git worktree add/remove` succeeds at `C:\uf\<32hex>` and leaves no registered worktree. Full now uses that short, real non-link source parent for only the checked-out committed source; child packages, raw data, candidate and final evidence remain under the original isolated attempt work root. The old failed `%TEMP%` probe directory is a local long-path cleanup residue; Git worktree metadata was pruned and no registered worktree remains. Parser and SelfTest pass; no post-commit Full claim is made. | scripts/Test-NuGetSignatureChain.ps1, direct detached-worktree probe, local SelfTest, this spec |
| EXEC | Real clean Full diagnostic | 1.00 | deadline/identity/adversarial fixtures and external delivery gate | retain the independently validated Full receipt as local diagnostic evidence only | Нет | Нет | On committed clean `8b65d1162812a70fd2d194100b4c9465c2e10546`, local Windows Full attempt `3` completed as `full-primary` success: Signature and Regression child receipts both succeeded, and the outer manifest contained exactly the 16 nested receipt/evidence files. The independent validator accepted the recursive tree. All temporary `C:\uf` worktrees were removed and `git status --short` remained empty after the run. This proves the current local wrapper/aggregation path, not the still-unimplemented deadline, native identity, adversarial-fixture or protected-merge contracts. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, temp evidence `…8b65d116…attempt-3-6b414545…`, local validator, this spec |
| EXEC | Full deadline envelopes | 0.97 | native path/file identity and adversarial child-lifecycle fixtures | enforce the declared 175-minute outer envelope, 65/95-minute children and ten-minute aggregation reserve without copying raw child streams to logs/files | Нет | Нет | Full now establishes one `DateTimeOffset` outer deadline, refuses to start Signature without 65+105 minutes remaining or Regression without 95+10, and refuses aggregation without its final ten-minute reserve. Each child runs via a process adapter that discards stdout/stderr, bounds it to its declared envelope and kills the child tree if it overruns; primary failure receipts remain valid nonzero exits and are still recursively validated. `SelfTest` proves an expired reserve is rejected, while the static contract anchors all declared limits. This is a bounded lifecycle foundation; native identity/link-count fixtures and real post-change Full evidence remain pending. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, this spec |
| EXEC | Full native child-tree isolation | 0.98 | adversarial child-lifecycle fixtures, then a committed-clean Full rerun | retain the local Windows-only Full boundary; non-Windows CI paths do not claim this local diagnostic invariant | Нет | Нет | The outer Full wrapper now rejects overlapping repository/final/candidate/work and child source/evidence/package roots, requires the exact `signature`/`regression` child-root grammar, and rejects links in each created root. On Windows it reads the volume serial, link count and file index through `GetFileInformationByHandle`: every final child and copied candidate file must be a non-link with link count `1`, unique identity within its tree, and a distinct identity across the pre-copy child and post-copy candidate trees. `SelfTest` creates a real hardlink and proves that the guard rejects it, as well as a nested-root overlap; parser, static TUnit contract and independent synthetic Full validation pass. This is not a new real Full receipt after the identity change. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Full adversarial receipt binding | 0.98 | committed-clean Full rerun and external CI evidence | keep child failure receipts legal, but treat fallback, lifecycle and recursive-binding corruption as outer failure | Нет | Нет | Full now accepts only primary Signature/Regression child receipts and binds the child native exit code to the child receipt outcome before aggregation. The self-test makes a valid recursive Full tree, then independently proves rejection of a fallback child, swapped child order, a wrong relative child root and an altered nested child receipt whose hash no longer matches the outer manifest. Existing synthetic cases continue to prove that a Signature failure is preserved while Regression evidence is included, and vice versa. Parser, `SelfTest`, the focused static TUnit contract and `git diff --check` pass. No new real Full receipt or remote CI result is claimed by this row. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, local SelfTest, local TUnit report, this spec |
| EXEC | Full child stream-drain correction | 0.98 | rerun committed-clean Full on the correction commit | use process-owned .NET stream draining; do not invoke PowerShell callbacks from a worker thread | Нет | Нет | The first attempt `4` on `39916385…` reached the isolated Signature child but the outer process terminated with `PSInvalidOperationException: There is no Runspace available to run scripts in this thread`: its event-handler callbacks were dispatched outside a PowerShell Runspace. The attempt is rejected and not counted; its detached `C:\uf\…` worktree was removed and no registered Full child worktree remains. The child adapter now drains both redirected streams with `CopyToAsync(Stream.Null)`, waits for both drains after natural exit or tree termination, and treats an unfinished or faulted drain as unproven. Parser, `SelfTest`, static TUnit contract and diff check pass; a new committed-clean Full attempt is still required. The failed attempt's isolated temp work/candidate/log residue is not evidence. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, failed local Full attempt `4` at `39916385…`, local SelfTest, local TUnit report, this spec |
| EXEC | Full post-lifecycle diagnostic | 1.00 | fresh exact-candidate CI result and protected-merge decision | record the local result as diagnostic evidence only; keep the external CI gate and governance decision independent | Нет | Нет | On committed clean `8d2aeb0579b211558b18f716ce424a0e824d8d51`, local Windows Full attempt `10` completed as `full-primary` `success`. Both isolated child receipts were primary successes, passed native file-identity/link checks, independent child validation and ordered recursive binding; their detached `C:\uf\…` worktrees were removed. A separate invocation of `Test-NuGetEvidencePublication.ps1` accepted the final Full tree: two ordered children (`Signature/signature`, `Regression/regression`) and exactly 16 manifest files. The caller worktree remained clean. This proves the deadline, stream-draining, identity and receipt lifecycle on the committed candidate; it does not turn the local wrapper into an external CI gate or authorize protected-branch changes. | scripts/Test-NuGetSignatureChain.ps1, scripts/Test-NuGetEvidencePublication.ps1, temp evidence `…attempt-10`, local validator, `git worktree list`, this spec |
| EXEC | Post-EXEC Android release-boundary correction | 0.99 | fresh candidate CI; release-event execution remains intentionally untriggered | replace the permissive ZIP check and wildcard overwrite with an attempt-qualified closed manifest and explicit collision-stopping release inputs | Нет | Нет | Review of the green candidate found that the release job accepted any two-entry archive if only one entry matched the APK regex, then uploaded every `*.apk` with `--clobber`. The correction makes the build publish two exact tag-qualified APK names plus `android-artifact-manifest.json`, exposes source/run/attempt/name metadata, validates artifact API metadata and the exact three-entry archive, rejects links, binds hashes and byte lengths from the manifest, and gives a pinned `softprops/action-gh-release` only the two validated explicit paths with `overwrite_files: false`. Focused static TUnit contract passed locally; final CI/re-review is still required and no release job is invoked from a PR. | .github/workflows/android-packaging.yml, src/Unlimotion.Test/CiReadmeMediaContract.cs, local TUnit report, this spec |
| EXEC | Regression failure-summary projection | 0.99 | fresh exact-candidate CI and Post-EXEC re-review | preserve only validated aggregate counters from a failed TUnit invocation; keep raw reports and all process output private and ephemeral | Нет | Нет | Two exact-source Windows Regression attempts failed with the same nonzero Unit command after all restore/build and both Headless runs had succeeded, while the direct local Unit suite passed 830/830. The prior adapter discarded the temporary raw reports before inspecting them on a nonzero exit, so the published failure evidence could not distinguish a test failure with a report from a host/process failure. It now attempts the existing strict report-shape and TRX parser only to carry `{discovered, passed, failed, skipped, durationMs}` into the sanitized failure record. If no valid report exists, the original generic `test-command-failed` record remains. No raw TRX/HTML, test names, standard streams, exception text or secrets are published; the self-test proves a failed record with counts is retained and the independent validator accepts it. This remains diagnostic hardening, not an external CI pass claim. | scripts/Test-NuGetSignatureChain.ps1, src/Unlimotion.Test/CiReadmeMediaContract.cs, this spec |
| EXEC | Final Post-EXEC re-review | 1.00 | explicit protected-merge decision only | close the technical gate on exact candidate without changing PR/governance state | Нет | Нет | Candidate `2e505492747b6b276089b064e7b4d4a291eb5641` passed the independent local `SelfTest`, focused executable TUnit contract and all observed external checks: `Signature`, Windows `Regression` (producer, independent validator, upload and enforcement), `android-build`, CodeQL actions/C#/JavaScript analysis and CodeQL. The Android correction is executed through build-side closed-artifact creation; release upload remains intentionally unexecuted on PR. Debian review confirmed its approved trust/permission/digest scope and retained the intentionally out-of-scope legacy release-asset replacement semantics. The worktree is clean; protected branch configuration, ready-state and merge remain human-owned operations. | GitHub Actions runs `30061601168`, `30061601181`, `30061601151`; local SelfTest and focused TUnit report; this spec |
