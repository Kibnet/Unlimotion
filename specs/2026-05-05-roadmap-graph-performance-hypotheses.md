...
| EXEC | H15: layout helper allocation cleanup | 0.70 | Нужен profiler, чтобы отделить GC benefit от layout latency noise | Не коммитить helper cleanup; не оптимизировать allocations без latency win | Нет | Да, пользователь подтвердил spec | Baseline H2 head: `small-chain` 916.70 ms / 32554.5 KB, `dense-shortcuts` 1660.85 ms / 96055.8 KB, `tree-with-blocks` 446.78 ms / 114261.1 KB. Post helper cleanup: `small-chain` 784.70 ms / 30654.9 KB, `dense-shortcuts` 3351.05 ms / 90854.4 KB, `tree-with-blocks` 812.51 ms / 108748.1 KB. Результат отклонен: allocation снизился, но latency на dense/tree регрессировал. | `src/Unlimotion/Views/Graph/RoadmapGraphBuilder.cs` (reverted) |
| EXEC | H7/H8/H0: debounce graph rebuild scheduler, skip unchanged scope resubscriptions, and UI timing counters | 0.84 | Нужны реальные user-action timings on production data | Закоммитить scheduler/subscription optimization; следующим шагом профилировать apply/render pipeline | Нет | Да, пользователь подтвердил spec | Baseline behavior по коду: `ScheduleUpdateGraph` делал immediate `Dispatcher.UIThread.Post`, поэтому spaced pulses после `RunJobs()` могли запускать несколько full rebuild подряд; `RegisterRoadmapScopeSubscriptions` пересоздавал scope subscriptions на каждый rebuild. Post behavior: one-shot `DispatcherTimer` 100 ms quiet window; UI test sends 3 `UpdateGraph` pulses spaced 40 ms with dispatcher jobs and observes 0 rebuilds before quiet window, then exactly 1 rebuild, and `RoadmapScopeSubscriptionRefreshCount` stays unchanged for same visible scope. Added counters: `RoadmapGraphUpdateCount`, `RoadmapLastBuildTime`, `RoadmapLastApplyProjectionTime`, `RoadmapScopeSubscriptionRefreshCount`. Roadmap UI tests: 21/21 PASS. | `src/Unlimotion/Views/GraphControl.axaml.cs`, `src/Unlimotion.Test/RoadmapGraphUiTests.cs` |
...
| H15 | Reduce layout helper allocations: reuse `VertexByTask`, remove extra layer materialization, manual `BuildOrderByVertex` | Same H15 baseline executable | `small-chain`: 916.70 ms / 32554.5 KB; `dense-shortcuts`: 1660.85 ms / 96055.8 KB; `tree-with-blocks`: 446.78 ms / 114261.1 KB | `tests\Unlimotion.Performance\artifacts\codex-h15-helper-post-build\Release\net10.0\Unlimotion.Performance.exe --warmup 1 --iterations 3` | `small-chain`: 784.70 ms / 30654.9 KB; `dense-shortcuts`: 3351.05 ms / 90854.4 KB; `tree-with-blocks`: 812.51 ms / 108748.1 KB | Rejected: allocations improve, but dense/tree latency regresses; no code committed | Build passed with existing warnings |
| H7/H8/H0 | One-shot debounced graph rebuild scheduler, unchanged-scope subscription reuse, and UI pipeline counters | Baseline by previous code path and behavior model: immediate UI post, coalescing only within one dispatcher turn; scope subscriptions recreated for every rebuild | Three spaced pulses under old mechanism could rebuild up to 3 times because each `RunJobs()` drained the posted callback; same-scope pulse rebuild still refreshed subscriptions | `src\Unlimotion.Test\artifacts\codex-roadmap-scheduler-tests-bin\Debug\net10.0\Unlimotion.Test.exe --treenode-filter "/*/*/RoadmapGraphUiTests/RoadmapGraph_UpdateGraphPulse_CoalescesQueuedRebuilds"` | 3 `UpdateGraph` pulses spaced 40 ms -> 0 rebuilds before quiet window, then exactly 1 rebuild; `RoadmapScopeSubscriptionRefreshCount` unchanged for same visible scope; build/apply timings > 0 | Accepted: reduces redundant UI rebuilds for event bursts, avoids subscription churn on same-scope rebuilds, and adds direct UI pipeline measurement counters | Full roadmap UI tests: `--treenode-filter "/*/*/RoadmapGraphUiTests/*"` -> 21/21 PASS |
