...
| EXEC | H7/H0: debounce graph rebuild scheduler and UI timing counters | 0.82 | Нужны реальные user-action timings on production data | Закоммитить scheduler debounce; следующим шагом профилировать apply/render pipeline | Нет | Да, пользователь подтвердил spec | Baseline behavior по коду: `ScheduleUpdateGraph` делал immediate `Dispatcher.UIThread.Post`, поэтому spaced pulses после `RunJobs()` могли запускать несколько full rebuild подряд. Post behavior: one-shot `DispatcherTimer` 100 ms quiet window; UI test sends 3 `UpdateGraph` pulses spaced 40 ms with dispatcher jobs and observes 0 rebuilds before quiet window, then exactly 1 rebuild. Added counters: `RoadmapGraphUpdateCount`, `RoadmapLastBuildTime`, `RoadmapLastApplyProjectionTime`. Roadmap UI tests: 21/21 PASS. | `src/Unlimotion/Views/GraphControl.axaml.cs`, `src/Unlimotion.Test/RoadmapGraphUiTests.cs` |
...
| H7/H0 | One-shot debounced graph rebuild scheduler and UI pipeline counters | Baseline by previous code path and behavior model: immediate UI post, coalescing only within one dispatcher turn | Three spaced pulses under old mechanism could rebuild up to 3 times because each `RunJobs()` drained the posted callback | `src\Unlimotion.Test\artifacts\codex-roadmap-scheduler-tests-bin\Debug\net10.0\Unlimotion.Test.exe --treenode-filter "/*/*/RoadmapGraphUiTests/RoadmapGraph_UpdateGraphPulse_CoalescesQueuedRebuilds"` | 3 `UpdateGraph` pulses spaced 40 ms -> 0 rebuilds before quiet window, then exactly 1 rebuild; build/apply timings > 0 | Accepted: reduces redundant UI rebuilds for event bursts and adds direct UI pipeline measurement counters | Full roadmap UI tests: `--treenode-filter "/*/*/RoadmapGraphUiTests/*"` -> 21/21 PASS |
