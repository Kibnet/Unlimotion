...
- Follow-up PR Unlimotion: `src/Directory.Packages.props` переведён с локального `8.1.0-settings-safety.20260921.4` на публичный `8.1.1`; restore подтвердил `WritableJsonConfiguration/8.1.1` в `project.assets.json`. Установленная версия приложения и реальные пользовательские настройки не изменялись; выпуск и установка остаются отдельными действиями.
...
При повторной проверке уже с публичным `8.1.1` полный main-набор дал 1112/1113: единственный сбой возник в существующем task-graph сценарии и не затронул settings/recovery. Тот же тест без изменения кода прошёл изолированно 1/1. Это не выдаётся за полностью зелёный full suite; окончательный чистый Windows CI остаётся обязательным gate PR.
...
| 2026-09-21 Follow-up: PR Unlimotion | Закрепить публичный `WritableJsonConfiguration` 8.1.1, повторить обязательную проверку и оформить PR без выпуска/установки приложения | Public restore, main/Headless/FlaUI validation и PR evidence | Не менять установленную версию и реальные пользовательские настройки | «Теперь оформи PR для unlimotion» | Этот PR и его CI |
