...
| EXEC | Исправить лишний резерв overflow-заголовков | 0.93 | Нет | Финальный diff/status review и обновить PR | Нет | Да, пользователь попросил исправить review finding | Убран постоянный `400px` reserve из расчёта вместимости вкладок; теперь резервируется только фактическая ширина overflow-кнопки и spacing. Добавлен regression assertion, что после кнопки не остаётся место, куда могла бы поместиться скрытая вкладка. Targeted overflow UI tests 8/8, AppAutomation Headless 31/31, desktop build PASS | `src/Unlimotion/Views/MainControl.axaml.cs`, `src/Unlimotion.Test/MainControlTabsOverflowUiTests.cs`, `specs/2026-06-04-main-tabs-overflow.md` |
