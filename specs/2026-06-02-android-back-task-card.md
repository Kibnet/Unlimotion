...
- Checks rerun:
  ...
  - Follow-up after real-device report, on emulator `emulator-5554` / Android API 34: `dotnet build src\Unlimotion.Android\Unlimotion.Android.csproj -c Debug -f net10.0-android -t:Install -p:AndroidDeviceSerial=emulator-5554 -p:UseSharedCompilation=false -v:minimal` -> PASS/install, warnings only.
  - Follow-up emulator key Back: first `adb shell input keyevent 4` keeps `com.Kibnet.Unlimotion/.MainActivity` foreground and opens the task card; second key Back keeps the same activity foreground and closes the task card.
  - Follow-up emulator edge Back gesture: `adb shell input swipe 5 1200 520 1200 250` keeps `com.Kibnet.Unlimotion/.MainActivity` foreground and opens the task card from the closed state.
  - Follow-up emulator logcat evidence: Android 13+ callback is registered with `mPriority=1000000`, matching `IOnBackInvokedDispatcher.PriorityOverlay`.
- Validation caveats:
  ...
  - Follow-up full CI-style test run: `dotnet test src\Unlimotion.Test\Unlimotion.Test.csproj -c Debug --no-restore -p:UseSharedCompilation=false -- --maximum-parallel-tests 1 --output Detailed` -> TIMEOUT after 15 minutes with no useful output; lingering `dotnet` process was stopped.
- Unrelated changes: no unrelated content diff detected; `src/Unlimotion.Android/Properties/AndroidManifest.xml` and `src/Unlimotion/App.axaml.cs` appear in `git status` only from line-ending/touch metadata and are not included in `git diff --name-only`.
- Needs human: нет.
- Residual risks / follow-ups: investigate the full `src\Unlimotion.Test` timeout/full-suite stability separately if it must be a strict local gate; existing Headless full-suite cross-thread failure remains outside this Android back gesture scope.
...
| EXEC | Проверить Android back path на эмуляторе после real-phone report | 0.95 | Нет по targeted Android flow | Коммит и push PR update | Нет | Да, пользователь попросил проверять на эмуляторе, телефон отключен | На API 34 эмуляторе confirmed: key Back и edge Back gesture открывают/закрывают карточку, приложение остается foreground; callback переведен на `PriorityOverlay`, чтобы получать Back раньше default Avalonia callback | `src/Unlimotion.Android/MainActivity.cs`, `src/Unlimotion.ViewModel/MainWindowViewModel.cs`, `src/Unlimotion.Test/MainWindowViewModelTests.cs`, `src/Unlimotion.Test/MainControlTaskCardLayoutUiTests.cs`, `specs/2026-06-02-android-back-task-card.md` |
