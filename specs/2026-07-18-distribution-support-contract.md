...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Commit `4fb574c4` исправил GNU/LLVM readelf portability: Distribution run `29831446698` подтвердил scope/contract, Windows, обе macOS архитектуры и Android dual-APK build PASS; Linux ожидаемо fail-closed на `NU3012`. Оба API jobs затем одинаково остановились до boot: `sdkmanager` установил `emulator`, но его каталог не был экспортирован в `PATH`. Новый TDD regression дал RED за 2.7 s; install step каждого API job теперь проверяет exact executable и добавляет `${ANDROID_SDK_ROOT}/emulator` в `$GITHUB_PATH`. Усиленный per-job Android regression PASS за 55.5 s, full local `All` PASS с 182 negatives за 106.9 s и independent review/rerun PASS за 53.1 s. Tests `29831446596` подтвердил Unit `830/830` и Headless `36/36`, AndroidPkg `29831446610` и CodeQL `29831446836` PASS. NuGet prerequisite draft в `rxs1` вернулся в `NEEDS-FIX` после нового architecture review и правится до стабильного re-review snapshot; implementation/approval закрыты. Затем требуются emulator-path replacement CI, prerequisite decision/approval/PR/merge, rebase PR #280, полный exact-SHA local gate и финальный native matrix/aggregate.
...
- Checks rerun: readelf fix commit `4fb574c4`; run `29831446698` contract/Windows/macOS/Android build PASS, Linux expected `NU3012`, затем API23/API36 одинаково fail-closed до boot из-за missing emulator PATH. Emulator-path change — expected RED 2.7 s, initial Android/`All` GREEN 76.1/111.2 s; strengthened per-job regression PASS 55.5 s, full `All` PASS 182 negatives / 106.9 s и independent review/rerun PASS 53.1 s; replacement run pending. Tests `29831446596` подтвердил Unit `830/830` и Headless `36/36`; AndroidPkg `29831446610` и CodeQL `29831446836` PASS. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
