...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Commit `1ef54245` перевёл emulator jobs на совместимый Ubuntu 22.04 и сделал loader diagnostic прозрачным: Distribution run `29836756606` подтвердил scope/contract, Windows, обе macOS архитектуры и Android dual-APK build PASS; Linux ожидаемо fail-closed на `NU3012`. Оба API jobs затем точно сообщили missing `libpulse.so.0`. Новый per-job host step использует стандартный 3-attempt APT pattern, устанавливает native `libpulse0` без recommendations и проверяет package state/exact SONAME. TDD RED 2.3 s, Android regression PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent review/reruns PASS (Android 51.8 s, `All` 182/91.4 s). Tests `29836756212` подтвердил Unit `830/830` и Headless `36/36`, AndroidPkg `29836756635` и CodeQL `29836756689` PASS. NuGet prerequisite draft в `rxs1` остаётся `NEEDS-FIX`; implementation/approval закрыты. Затем требуются libpulse replacement CI, prerequisite decision/approval/PR/merge, rebase PR #280, полный exact-SHA reset и финальный native matrix/aggregate.
...
| MEDIUM | Android emulator libpulse runtime | Run `29836756606` в API23/API36 точно сообщил missing `libpulse.so.0` из qemu loader на Ubuntu 22.04 | Установить native `libpulse0` без recommendations через bounded APT retry; проверить package state и `/usr/lib/x86_64-linux-gnu/libpulse.so.0` в каждом job | fixed in working tree; TDD RED 2.3 s, Android 43.5 s и `All` 182/103.8 s PASS; independent Android 51.8 s + `All` 182/91.4 s/review PASS; replacement pending |
...
- Checks rerun: host/probe fix commit `1ef54245`; run `29836756606` contract/Windows/macOS/Android build PASS, Linux expected `NU3012`, затем API23/API36 точно fail-closed на missing `libpulse.so.0`. Host-runtime change — expected RED 2.3 s, Android regression PASS 43.5 s и `All` PASS с 182 negatives за 103.8 s; independent Android 51.8 s и `All` 182/91.4 s, review PASS; replacement pending. Tests `29836756212` подтвердил Unit `830/830` и Headless `36/36`; AndroidPkg `29836756635` и CodeQL `29836756689` PASS. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
