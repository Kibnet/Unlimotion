...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Commit `75e0f9b4` разделил raw nupkg SHA-512 и NuGet logical `contentHash`; local Android/full gates и independent review были PASS. Replacement Distribution run `29827483394` сохранил fail-closed boundary и выявил Windows-only fixture defect до native matrix: MSYS path был записан в JSON, тогда как native Python verifier канонизировал тот же argument как `C:\...`. Fixture теперь получает feed/packages paths через тот же `bash` + `python3` runtime; PowerShell AST, targeted Android PASS за 94.4 s и full local `All` PASS с 182 negatives за 144.9 s. Companion AndroidPkg `29827483399` и CodeQL `29827483419` PASS; Tests `29827483456` завершился `829/830` из-за известного one-off Avalonia `PanelContainerGenerator.InsertContainer` flake и обязан быть rerun на новом head. NuGet prerequisite draft в `rxs1` проходит повторные role reviews; implementation/approval остаются закрыты до исправлений и independent PASS. Затем требуются replacement CI, prerequisite approval/PR/merge, rebase PR #280, полный exact-SHA local gate и финальный native matrix/aggregate.
...
| MEDIUM | Android fixture cross-runtime paths | Distribution run `29827483394` на Windows записал MSYS `/d/...` package root в fixture, но native Python получил тот же argv как `C:\...` и корректно отклонил source/root mismatch до native matrix | Канонизировать feed/packages fixture paths тем же `bash` + `python3 pathlib.resolve()` runtime, который исполняет verifier; fail on empty/multiline probe result | fixed in working tree; independent path-semantics review PASS, Android 94.4 s и `All` 182/144.9 s PASS; replacement run pending |
...
- Checks rerun: hash-domain change — expected RED 25.7 s, corrected Android regression PASS 63.8/54.2 s, full `All` PASS 182/134.2 s и independent review PASS; commit `75e0f9b4` pushed. Run `29827483394` затем выявил только cross-runtime fixture-path mismatch на Windows до native jobs. После канонизации через verifier Python runtime PowerShell AST PASS, Android PASS 94.4 s и full `All` PASS 182/144.9 s; independent review PASS. Tests `29827483456` получил известный one-off Avalonia flake `829/830`, AndroidPkg `29827483399` и CodeQL `29827483419` PASS. Replacement run и Tests rerun pending. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
