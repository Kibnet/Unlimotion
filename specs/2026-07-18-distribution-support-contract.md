...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Exact-head run `29824937816` на `12b191cf` подтвердил scope/contract/Windows и обе macOS архитектуры; Linux ожидаемо fail-closed на `NU3012`, а Android дошёл дальше прежнего `NU1102`, собрал APK/provenance и выявил отдельный verifier defect: NuGet logical `contentHash` ошибочно сравнивался с raw SHA-512 signed nupkg. TDD fixture воспроизвёл RED; hash domains разделены, targeted Android suite PASS за 63.8 s и после финальной перестановки helper повторно за 54.2 s, full local `All` PASS с 182 negatives за 134.2 s; independent code/test review PASS, commit/push и новый tracked exact-head run pending. Companion Tests `29824937839`, AndroidPkg `29824937832` и CodeQL `29824937917` PASS. NuGet prerequisite draft в `rxs1` проходит повторные role reviews, которые уже нашли новые implementability/security gaps; implementation/approval остаются закрыты до исправлений и independent PASS. Затем требуются prerequisite approval/PR/merge, rebase PR #280, полный exact-SHA local gate и финальный native matrix/aggregate.
...
- Checks rerun: hash-domain change — expected RED 25.7 s, corrected Android regression PASS 63.8 s и повторно 54.2 s после перестановки helper, full `All` PASS 182 negatives / 134.2 s, Bash syntax, PowerShell AST и `git diff --check` PASS; independent code/test review PASS. Replacement native run выполняется после commit/push. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
