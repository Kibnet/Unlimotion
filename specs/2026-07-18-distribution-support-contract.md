...
- Текущая фаза: `EXEC / reviewed harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт, первый native distribution run `29794200613` выполнен. Android local-feed, configured-smoke evidence и truthful README workaround исправлены в working tree и прошли independent reviews; текущий static contract PASS с 172 negative fixtures. До final Stage-3 reset остаются отдельное точное approval, commit/implementation PR/merge для `reactiveui-signature-chain-remediation`, затем rebase PR #280, полный exact-SHA local gate и повторный native matrix/aggregate.
...
- Связанные артефакты:
  ...
  - dependency-security prerequisite child spec: `specs/2026-07-21-reactiveui-signature-chain-remediation.md` (reviewed `PASS` в отдельном `rxs1`, exact approval pending);
...
| NuGet signature prerequisite sequencing | agent; separate child spec final reviews PASS | Отдельное exact approval -> prerequisite PR/merge -> Stage-3 rebase/full reset; signature verification bypass запрещён | 1.00 | Insecure restore, mixed dependency scope или недостоверный Linux support verdict | Да; отдельный exact approval pending |
...
- Residual gates: exact approval/PR/merge dependency-security prerequisite; полный reset local gate после rebase; повторный Unix/Windows/Android native matrix и aggregate на уже открытом PR #280; final Post-EXEC review и merge.

### Post-EXEC Review

- Статус: `NEEDS-FIX`; draft PR #280 открыт. Distribution run `29794200613` дал PASS для scope/contract и Windows на elevated runner, выявил Android local-feed deletion, остановил Linux до smoke на revoked NuGet signature chain и остановил macOS на первом portable launch в `/Tasks`; Windows cross-platform impact установлен последующим source analysis, а не failure этого job. Android/feed, configured-smoke и truthful README workaround fixes реализованы в working tree. NuGet prerequisite существует только как untracked draft в отдельном worktree `rxs1`: Role-Based/Post-SPEC final re-reviews PASS; отдельное точное approval, spec commit, implementation PR и merge ещё pending. Любое новое evidence до commit/final reset считается устаревшим.
...
- Decision: завершить fail-closed Android feed/configured-smoke/README fixes, зафиксировать их отдельными commits и повторить native matrix после prerequisite. Product `Program.cs` не менять в Stage 3: unconfigured first-run defect `/Tasks`/current-drive root передан Stage 5. Linux trust-chain draft `specs/2026-07-21-reactiveui-signature-chain-remediation.md` в отдельном `rxs1` прошёл Role-Based/Post-SPEC PASS; следующий gate — отдельное точное approval, после которого разрешены spec commit/implementation PR/merge. Stage 4 начинать нельзя. Android run не подтвердил API-23 incompatibility, поэтому этот условный ASK-HUMAN не сработал.
...
- Evidence inspected: всё ранее перечисленное build-isolation evidence плюс четыре раздельных PR #280 workflow run. Distribution `29794200613`: `distribution-scope`, `distribution-contract` и elevated `windows-x64-native` PASS; macOS x64/arm64 завершили candidate build, ad-hoc sealing и evidence envelope, но validation остановилась на первом portable launch в `/Tasks` до Setup install и `pkgutil` signature classification, поэтому последующие package/sign classifications не выполнялись; standalone Android после удаления tracked feed сделал NU1603 fallback к public `NodifyAvalonia 6.6.0`; Linux restore упал с `NU3012 Revoked` до начала smoke. Companion Tests `29794200612`, AndroidPkg `29794200679` и CodeQL `29794200684` PASS; отдельный AndroidPkg сохранил tracked local package. Windows PASS получен на elevated runner, а разрешение default в корень текущего диска (например, `C:\Tasks`) подтверждено source analysis и этим job не проверено. Current checks after harness/README fixes: полный static All PASS с 172 negative fixtures, включая WorkflowSecurity 69 negatives и Evidence 46 negatives; Android script suite PASS за 55.4 s; full exact-SHA reset pending after docs/commit.
...
| HIGH | Android local feed | Standalone builder удалял tracked Nodify и принимал public fallback; ignored/hierarchical feed, exact-ID source substitution и dirty-source relabeling могли обходить двухпакетный claim | Output-scoped flat feed только из verified `HEAD` Nodify + current LibGit; exact source mapping, post-restore SHA-512/source proof, recursive no-extra/symlink closure, sourceSha=clean HEAD/index/submodules и executable negatives | fixed in working tree; Android suite PASS 55.4 s, native rerun pending |
...
| BLOCKER | Linux NuGet trust chain | Fresh Linux restore отклоняет текущие ReactiveUI/Splat packages с `NU3012 Revoked` до smoke; Android/Debian workflows также содержат signature-verification bypass | Получить exact approval reviewed-PASS draft в `rxs1`, затем выполнить spec commit/implementation PR/merge и после merge rebase/full Stage-3 reset | draft untracked; reviews PASS, approval/commit/PR/merge pending |
...
- Checks rerun: прежнее evidence остаётся baseline. На текущих native harness/README fixes полный static All PASS с 172 negative fixtures за 95.6 s, включая WorkflowSecurity 69 negatives и Evidence 46 negatives; Android build-script regression PASS за 55.4 s, PowerShell/bash parsers, paired README contract и `git diff --check` PASS. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и native matrix должны быть повторены после окончательного tracked commit.
...
- Needs human: сейчас требуется отдельное точное `Спеку подтверждаю` для reviewed-PASS untracked draft `specs/2026-07-21-reactiveui-signature-chain-remediation.md` в отдельном `rxs1`; затем всё ещё останутся spec commit, implementation PR и merge. Прежние approvals его не покрывают. API-23 ASK-HUMAN не достигнут. Stage-5 product data-path child spec потребует своё approval позже.
...
Build-isolation EXEC выполнен в exact allowlist commit `12d0bba9`. Draft PR #280 и первый native matrix уже выполнены; Stage-3 delivery всё ещё требует merge dependency-security prerequisite, rebase/final-candidate docs commit, полный exact-SHA reset local gate, повторный native matrix/aggregate, final review и merge.
...
| EXEC | Исправить Android local feed, configured desktop smoke и README overclaim | 0.99 | Full reset/native rerun | Завершить review/docs, commit/push | Нет | Не применимо | Isolated HEAD-bound two-package feed, NU1603 fail-closed, seeded TaskStorage disclosure, late-root negatives и EN/RU explicit-writable-path caveat закрывают harness/docs defects без `Program.cs` scope drift; product fix передан Stage 5 | Android/Windows/Linux/macOS validators, aggregate, README contract tests |
| SPEC | Подготовить NuGet trust-chain prerequisite | 1.00 | Separate exact approval | Запросить approval, затем commit/implementation PR/merge | Да | Ещё не обращались | Validation, security и architect/governance final re-reviews PASS; Linux failure не является API-23/product compatibility и не может исправляться отключением signature verification | `specs/2026-07-21-reactiveui-signature-chain-remediation.md` в `rxs1`, reviewer verdicts |
