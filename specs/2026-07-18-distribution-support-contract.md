...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Commit `066ca4f9` исправил Windows cross-runtime fixture paths: Distribution run `29829779408` подтвердил scope/contract, Windows и обе macOS архитектуры PASS; Linux ожидаемо fail-closed на `NU3012`. Android собрал оба APK, но artifact validator выбрал системный GNU `readelf` и передал LLVM-only alias `--dyn-symbols`; поэтому API cells корректно skipped. Новый TDD regression дал RED за 1.8 s; вызов заменён на общий GNU/LLVM `--dyn-syms`, Android suite PASS за 70.4 s и full local `All` PASS с 182 negatives за 133.7 s. Companion Tests `29829779180` PASS Unit `830/830` + Headless `36/36`, AndroidPkg `29829779171` и CodeQL `29829779164` PASS. NuGet prerequisite draft в `rxs1` прошёл author checks и находится в новом independent re-review; implementation/approval остаются закрыты до трёх PASS. Затем требуются readelf replacement CI, prerequisite decision/approval/PR/merge, rebase PR #280, полный exact-SHA local gate и финальный native matrix/aggregate.
...
| MEDIUM | Android artifact readelf portability | Run `29829779408` собрал оба APK, затем системный GNU `readelf` отклонил LLVM-only alias `--dyn-symbols`; API cells были fail-closed skipped | Использовать общий GNU/LLVM `--dyn-syms`; добавить exact-two-call assertion и запрет непереносимого alias | fixed in working tree; expected RED 1.8 s, Bash syntax, Android 70.4 s, reviewer rerun 97.9 s и `All` 182/133.7 s PASS; independent review PASS, replacement run pending |
...
- Checks rerun: cross-runtime path fix commit `066ca4f9`; run `29829779408` contract/Windows/macOS PASS и Linux expected `NU3012`, затем Android artifact validator fail-closed на GNU `readelf --dyn-symbols`. Tests rerun `29829779180` PASS Unit `830/830` + Headless `36/36`; AndroidPkg `29829779171` и CodeQL `29829779164` PASS. Readelf portability change — expected RED 1.8 s, Bash syntax PASS, Android regression PASS 70.4 s, independent reviewer rerun PASS 97.9 s, full `All` PASS 182 negatives / 133.7 s; review PASS, replacement run pending. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
