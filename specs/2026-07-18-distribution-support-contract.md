...
- Текущая фаза: `EXEC / exact-head harness remediation, blocked by dependency-security prerequisite`; LF/blob-parity и Desktop build isolation реализованы, draft PR #280 открыт. Commit `0417ee22` исправил emulator PATH: Distribution run `29834142027` подтвердил scope/contract, Windows, обе macOS архитектуры и Android dual-APK build PASS; Linux ожидаемо fail-closed на `NU3012`. Оба API jobs нашли emulator, но одинаково завершились `127` в version probe. Separate ELF scan official Linux packages 36.4.9, stable 36.6.11 и preview 37.2.1 подтвердил external `libtiff.so.5`: Ubuntu 24.04 предоставляет только `libtiff6`, Ubuntu 22.04 — совместимый `libtiff5`. Оба emulator-only jobs переводятся на supported `ubuntu-22.04`; version helper теперь сохраняет loader stderr/exit. Host/probe TDD RED 1.6/1.9 s, Bash syntax + Android regression PASS за 46.8 s и `All` PASS с 182 negatives за 108.8 s; independent targeted 50.2 s и `All` 182/100.9 s, review PASS. Tests `29834141673` подтвердил Unit `830/830` и Headless `36/36`, AndroidPkg `29834142085` и CodeQL `29834141718` PASS. NuGet prerequisite draft в `rxs1` остаётся `NEEDS-FIX`; implementation/approval закрыты. Затем требуются emulator-host replacement CI, prerequisite decision/approval/PR/merge, rebase PR #280, полный exact-SHA reset и финальный native matrix/aggregate.
...
- Checks rerun: emulator PATH fix commit `0417ee22`; run `29834142027` contract/Windows/macOS/Android build PASS, Linux expected `NU3012`, затем API23/API36 прошли PATH guard и одинаково fail-closed с loader exit `127`. Separate three-package ELF scan подтвердил `libtiff.so.5` host gap. Host/probe changes — expected RED 1.6/1.9 s, Bash syntax + Android regression PASS 46.8 s, `All` PASS 182 negatives / 108.8 s; independent targeted 50.2 s и `All` 182/100.9 s, review PASS; replacement pending. Tests `29834141673` подтвердил Unit `830/830` и Headless `36/36`; AndroidPkg `29834142085` и CodeQL `29834141718` PASS. Paired README contract не затронут. Полный exact-SHA README/entrypoint/build/Unit/Headless gate и final native matrix повторяются после prerequisite merge/rebase.
