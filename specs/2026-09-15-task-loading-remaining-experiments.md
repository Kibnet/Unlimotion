...
| PR REVIEW | Guarded atomic-write rollback | 0.99 | Финальный remote CI/re-review | Сохранить fix | Нет | Codex review P1 на `df415dbc` | Guarded migration write удерживает displaced backup до всех generation checks. При любой post-replace ошибке текущий target атомарно переносится в quarantine: если SHA-256 совпадает с миграционными байтами, восстанавливаются точные displaced bytes; если target уже содержит более поздний внешний edit, он возвращается без перезаписи. Конкурентно воссозданный target всегда выигрывает, а невозможность безопасного rollback останавливает миграцию вместо retry по потерянным данным | `GuardedSave_RestoresExternalEditDisplacedDuringAtomicReplace` + `GuardedSave_RejectsExternalInvalidationDuringLiveGraphPublication` + `GuardedSave_RollsBackWhenPostWritePublicationFails`; FileStorageTaskStatusTests 37/37; UnifiedTaskStorageMigrationRegressionTests 6/6; Debug headless 41/41 |
| CI FIX | Status migration watcher ownership | 0.99 | Финальный remote CI/re-review | Сохранить fix | Нет | CI `35153989963`: 29/41 headless упали при startup migration | Legacy status migration раньше перезаписывала task-файлы через прямой `File.WriteAllTextAsync`; raw watcher верно считал эти записи внешними и пять раз инвалидировал guarded availability migration. Теперь эти записи проходят через тот же SHA-256 ownership contract и атомарную temp/replace запись, что и обычный storage save. Внешние отличающиеся байты по-прежнему снимают confirmation и инвалидируют graph | `UnifiedTaskStorage_Init_IgnoresRawWatcherEchoesFromStatusMigration`; UnifiedTaskStorageMigrationRegressionTests 7/7; FileStorageTaskStatusTests 37/37; Debug headless 41/41 |
| PR REVIEW | Displaced source identity and alias takeover | 0.99 | Финальный remote CI/re-review | Сохранить fixes | Нет | Codex review P1/P2 на `1f0ec493` | Live graph хранит SHA-256 тех же байтов, из которых десериализован каждый source. Guarded write сверяет текущий source и displaced backup с этим hash; несовпадение откатывает собственную запись и сохраняет внешние байты. При смене alias ID старый ID получает `Saved` с актуальным snapshot, если его уже занял другой source, и `Removed` только при фактическом исчезновении. Семантически равный raw refresh обновляет source hash без ложных retry | `GuardedSave_RestoresUnsignaledExternalEditDisplacedDuringAtomicReplace`; `GuardedSave_AcceptsHashRefreshedBySemanticallyEquivalentRawEdit`; `RawAliasIdChange_RepublishesOldIdTakenOverByAnotherSource`; FileStorageTaskStatusTests 40/40; затронутые storage-наборы 83/83; Release headless 41/41 |
| PR FINAL | Post-review exact benchmark | 0.99 | Финальный remote CI/re-review | Сохранить PR и пройти remote gates | Нет | Пользователь запросил проверить отсутствие ухудшения | Чистая серия `main@f58bb6fc`→candidate: 5 пар/10 успешных процессов/30 измерений. Ready paired median: startup −11,28%, Space B −25,75%, Space A −29,97%; полный маршрут −21,07% (mean −21,28%), быстрее 5/5. Action guardrails: startup −3,4 ms, B +2,11%, A +2,32%. Peak private median ниже на 8,85–15,10%. Дополнительная hash-проверка не уничтожила E2E-пользу и осталась внутри first-action guardrails | `TestResults/loading-remaining/final-review-hash-vs-main/measurements.jsonl`; clean baseline app; exact candidate app |
| CI FIX | Settle owned status-migration watcher burst | 0.99 | Повторный remote CI/re-review | Сохранить fix | Нет | CI `35159968706`: 1/1053 main failure и каскад 35/41 headless | Windows Debug доставлял часть raw echoes атомарной legacy status migration уже после построения live graph. Если status migration реально изменила файлы, startup теперь ждёт устойчивое raw-поколение перед authoritative graph read. Одновременные внешние правки входят в этот read, а более поздние по-прежнему инвалидируют новую generation. Обычный уже мигрированный startup задержку не получает | CI regression stress 10/10; UnifiedTaskStorageMigrationRegressionTests Debug 7/7; Debug headless 41/41 |
| CI FIX | Require a full watcher-debounce quiet window | 0.99 | Повторный remote CI/re-review | Сохранить fix | Нет | CI `35161541228`: main 1053/1053, но 36/41 headless упали на более позднем хвосте того же FSW burst | Quiescence больше не завершается после первого тихого 200-мс среза. Требуются пять последовательных стабильных срезов, то есть полное 1-секундное окно watcher debounce; новое raw-поколение обнуляет счётчик. Максимальное ожидание ограничено 2 секундами, а generation guard остаётся окончательной защитой | `ExternalAliasIdChange_ReplacesOldDesktopProjection` Debug 10/10 в отдельных процессах; полный Debug headless 41/41 |
| CI FIX | Active-write watcher ownership | 0.99 | Точный remote CI/re-review нового head | Сохранить архитектурный fix; удалить оба time-based workaround | Нет | CI `35163196924`: main 1052/1053 и headless 23/41 показали, что даже 1-секундное quiet window не ограничивает хвост Windows FSW | Ownership теперь привязан к жизненному циклу фактической атомарной записи: raw event, синхронно пришедший пока `File.Replace` показывает displaced/отсутствующий target, считается собственным; сразу после persistence снова действует точное SHA-256 сравнение. `finally` завершает ownership и при ошибке, а TTL применяется только после завершения записи. Это сохраняет invalidation для внешней правки между persistence и публикацией live graph. Оба эвристических ожидания после status migration удалены, исходный короткий quiescence восстановлен | Status regression Debug 10/10 в отдельных процессах; UnifiedTaskStorageMigrationRegressionTests 7/7; FileStorageTaskStatusTests 40/40; полный Debug headless 41/41 на правой панели |
| PR REVIEW | Case-sensitive source identity | 0.99 | Linux remote regression | Сохранить fix | Нет | Codex review P2 на `36925928` | Source hashes, pending watcher work, own-write confirmations и live-graph file identity используют platform path semantics: case-insensitive на Windows, case-sensitive на Unix. Поэтому `foo` и `Foo` не делят hash/ownership на case-sensitive filesystem, а Windows сохраняет прежнее поведение. Добавлена Linux regression с guarded save двух case-distinct файлов | FileTaskStorageReadContractTests 10/10 локально; FileStorageTaskStatusTests 40/40; FileTaskStorageRecoverableMutationTests 14/14; Linux test должен пройти в exact-head CI |
| PR REVIEW | Deferred in-write validation and reconciliation identity | 0.99 | Точный remote CI/re-review | Сохранить fixes | Нет | Codex P1/P2 на `284e6ecf` | Raw event во время атомарной записи больше не отбрасывается: тип события сохраняется, а после persistence target сверяется с ожидаемым SHA-256. Несовпадение атомарно публикует pending change и invalidation до generation check, поэтому guarded migration откатывается и сохраняет внешние байты. Во время full rescan alias разрешается через последний опубликованный graph, пока mutable mapping перестраивается. Final cache reconciliation сравнивает error paths теми же platform semantics | Три детерминированных regression по 10 повторов, 30/30; FileStorageTaskStatusTests 43/43. Unix case-distinct reconciliation выполняется в exact-head CI |
