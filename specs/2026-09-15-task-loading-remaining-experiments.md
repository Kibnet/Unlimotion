...
- Перед финальным merge gate ветка rebased без конфликтов на `f58bb6fc`. После финального watcher interleaving fix авторитетный benchmark актуального main против точного candidate `e57a85e5` завершил 5/5 пар и 12/12 процессов: startup Ready median 11,549→10,148 s, median paired delta −9,42% (5/5); Space B 7,270→5,299 s, −23,57% (5/5); возврат Space A 5,874→3,983 s, −28,51% (5/5). Полный маршрут быстрее 5/5, median paired delta −19,11%; Ready+Action до фактического появления relation editor: −8,50% / −14,81% / −15,48%, везде 5/5. Чистое Action проходит short-action guardrail max(5%, 100 ms): median absolute delta +25/+24/+48 ms. Peak private memory ниже на 6,55–15,46% median paired. Dataset fingerprint `49906522...567135A`; exe SHA-256 main/candidate `D80E10E3...17FE1D6`/`FBBD6CCD...4DB46A9`. Артефакт: `final-head-e57a85e5-vs-main/measurements.jsonl`.
...
| PR FINAL | Rebase и точный final-head benchmark | 0.99 | Только CI нового remote head | Commit/push и проверить CI | Нет | Пользователь запросил rebase и три merge gate | Rebase на `f58bb6fc`; exact `e57a85e5` main→candidate: Ready −9,42%/−23,57%/−28,51%, полный маршрут −19,11%, 5/5; Ready+Action −8,50%/−14,81%/−15,48%, 5/5; Action guardrail пройден; 12/12 процессов | `final-head-e57a85e5-vs-main/measurements.jsonl` |
| PR FINAL | Precise raw watcher refresh | 0.99 | Нет | Сохранить | Нет | Пользователь явно запросил оптимизацию raw watcher | Precise события применяются инкрементально; unknown/global/duplicate остаются full-rescan. Изолированный вклад: полный маршрут −8,36%, startup −7,84%, B +0,86%, A −12,58%; 12/12 процессов | `final-raw-watcher-contribution/measurements.jsonl`, FileStorageTaskStatusTests 27/27 |
