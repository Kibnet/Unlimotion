...
- Перед финальным merge gate ветка rebased без конфликтов на `f58bb6fc`. Авторитетный benchmark актуального main против точного candidate `61fdaef5` завершил 5/5 пар и 12/12 процессов: startup Ready median 11,962→9,879 s, median paired delta −17,41% (5/5); Space B 7,089→5,124 s, −24,14% (5/5); возврат Space A 5,855→4,178 s, −27,53% (5/5). Полный маршрут быстрее 5/5, median paired delta −22,69%; Ready+Action до фактического появления relation editor: −16,95% (5/5) / −17,04% (4/5) / −13,10% (5/5). Чистое Action не нарушает guardrail: mean/median +3,70%/−2,40%, +2,82%/+0,84%, −1,43%/+0,55%; median absolute delta −7/+39/+25 ms. Peak private memory ниже на 5,59–15,00% median paired. Dataset fingerprint `49906522...567135A`; exe SHA-256 main/candidate `D80E10E3...17FE1D6`/`E9D2EB60...04B981`. Артефакт: `final-head-corrected-action-vs-main/measurements.jsonl`.
...
| PR FINAL | Rebase и точный final-head benchmark | 0.99 | Только CI нового remote head | Commit/push и проверить CI | Нет | Пользователь запросил rebase и три merge gate | Rebase на `f58bb6fc`; exact `61fdaef5` main→candidate: Ready −17,41%/−24,14%/−27,53%, полный маршрут −22,69%, 5/5; Ready+Action −16,95%/−17,04%/−13,10%; Action guardrail пройден; 12/12 процессов | `final-head-corrected-action-vs-main/measurements.jsonl` |
| PR FINAL | Precise raw watcher refresh | 0.99 | Нет | Сохранить | Нет | Пользователь явно запросил оптимизацию raw watcher | Precise события применяются инкрементально; unknown/global/duplicate остаются full-rescan. Изолированный вклад: полный маршрут −8,36%, startup −7,84%, B +0,86%, A −12,58%; 12/12 процессов | `final-raw-watcher-contribution/measurements.jsonl`, FileStorageTaskStatusTests 26/26 |
| PR FINAL | Harness и regression gates | 0.99 | Новый remote CI | Commit/push | Нет | Обязательный pre-merge пункт | Устранено закрытие открывшейся pane при задержке binding; smoke 4/4, две финальные серии по 12/12; main 1037/1037, headless 40/40 | `final-harness-smoke2`, локальные test reports |
