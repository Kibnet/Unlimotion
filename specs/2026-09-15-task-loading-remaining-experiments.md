...
| PR REVIEW | Aliased source ID replacement | 0.99 | Финальный remote CI/re-review | Сохранить fix | Нет | Codex review P1 на `55a0cf68` | После precise refresh delayed callback повторно разрешает mapping `source file -> current domain ID`, загружает фактический alias source и публикует ordered removal old ID + save new ID. Storage regression 29/29, новый headless UI-сценарий green, полный headless 41/41 | Final follow-up после `55a0cf68` |
