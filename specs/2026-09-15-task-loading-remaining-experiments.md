...
| N11 | Отдельный процесс с DOTNET_TieredCompilation=0 против штатной конфигурации, тот же binary | Startup/A→B/B→A плюс 10 минут одинаковых действий после прогрева | Конфигурация только процесса теста, восстановление env; не менять установленный runtime; throughput/паузы/CPU и RSS |
...
| EXEC | N11 tiered compilation off | 0.99 | Нет | Отклонить | Нет | Не применимо | Тот же binary, только process env. Startup хуже mean 19,79%, median 21,46%, 0/5; B хуже 3,65%/6,06%; A хуже 24,90%/22,06%, 0/5. Нарушены startup/guardrails, long throughput не нужен. Env восстановлен | measure-task-loading.ps1; `n11-tiered-off-e2e` |
