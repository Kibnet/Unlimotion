...
| EXEC | Planning duration height alignment | 0.95 | Нет | Report follow-up result | Нет | Да, пользователь попросил выровнять `Планируемая длительность` по высоте с датами | Added a regression assertion for matching planning value heights; reproduced `begin=44`, `duration=36`, `end=44`; set duration editor height to `44`; targeted task-card UI tests passed 5/5 | `src/Unlimotion/Views/MainControl.axaml`, `src/Unlimotion.Test/MainControlTaskCardLayoutUiTests.cs`, `specs/2026-06-02-task-card-desktop-planning-repeater-polish.md` |
| EXEC | CI layout follow-up for desktop planning row | 0.88 | GitHub rerun result after push | Run targeted layout tests and push PR update | Нет | Да, GitHub CI failed on PR after Android follow-up | CI runner wrapped the third desktop planning group at `1400x900`; reducing regular planning group width keeps the existing one-row contract without changing Android back behavior | `src/Unlimotion/Views/MainControl.axaml`, `src/Unlimotion/Views/MainControl.axaml.cs`, `specs/2026-06-02-task-card-desktop-planning-repeater-polish.md` |
