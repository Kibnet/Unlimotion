# SPEC: Исполняемые step definitions для Telegram callback-действий

## Контекст

Текущий BDD layer уже исполняет три сценария:

- `SC-0015-002` через `SD-0001..SD-0004` и `TS-0026`;
- `SC-0014-002` через `SD-0005..SD-0008` и `TS-0027`;
- `SC-0014-001` через `SD-0009..SD-0012` и `TS-0028`.

Следующий безопасный кандидат для расширения `/storm:bdd-implement` — `SC-0014-003`:

- Story: `ST-0014`
- Feature: `GF-014`
- Rule: `GR-040`
- Scenario: `SC-0014-003`
- Existing test evidence: `TS-0023`
- Feature file: `features/storm/st-0014-telegram-bot.feature`
- Existing test file: `src/Unlimotion.Test/TelegramBotCallbackCoverageTests.cs`

Сценарий уже подтвержден существующими TUnit тестами и не требует реального Telegram API, credentials, polling, network, UI или Git remote:
...
Добавить исполняемые BDD step definitions для `SC-0014-003`, сохранив существующее поведение, acceptance criteria, `.feature` wording и test annotations.
...
- добавить step definitions для четырех шагов `SC-0014-003`;
- добавить отдельный executable spec test, который запускает сценарий из `.feature` файла;
- вынести общую проверочную логику `TelegramBotCallbackCoverageTests` в переиспользуемый test contract/helper, если это нужно для устранения дублирования;
...
- расширять command/auth `SC-0014-001` или Git timer `SC-0014-002`;
- исправлять Android/iOS `NETSDK1147` blocker.

## План реализации

1. Вынести или переиспользовать deterministic callback checks из `TelegramBotCallbackCoverageTests`.
2. Добавить `TelegramCallbackStepDefinitions` в `src/Unlimotion.Test/StormBdd`.
3. Добавить executable spec test для `SC-0014-003`.
4. Синхронизировать `docs/product/storm.json`:
   ...
   - связать `SC-0014-003` с новым executable test и step definitions;
   - обновить BDD coverage metrics до `4/45` step-executable scenarios.
5. Обновить reports в `docs/product/reports`.
6. Не трогать production code и existing test annotations.

## Acceptance Criteria

- `SC-0014-003` получает исполняемые step definitions для всех четырех Gherkin steps.
- Новый executable spec test запускает сценарий из `features/storm/st-0014-telegram-bot.feature`.
- Существующие tests из `TS-0023` остаются рабочими и не теряют annotations.
...
- Behavior coverage reports отражают четвертый step-executable scenario.
...
dotnet test src\Unlimotion.Test\Unlimotion.Test.csproj -c Release --no-build --no-restore -- --treenode-filter "/*/*/StormTelegramCallbackExecutableSpecTests/*" --output Detailed
...
- обнаружится расхождение между `.feature`, `storm.json` и фактическим `TS-0023`, которое нельзя исправить artifact sync без изменения поведения.

## Риски

- Сценарий агрегирует несколько callback paths, поэтому step definition может стать слишком широкой. Снижение риска: использовать существующий deterministic callback contract и не добавлять real Telegram integration.
...
| --- | --- | --- |
| 2026-06-19 | SPEC создана | `SC-0014-003`, `TS-0023`, `features/storm/st-0014-telegram-bot.feature` |
| 2026-06-19 | EXEC выполнен | `SD-0013..SD-0016`, `TS-0029`, `StormTelegramCallbackExecutableSpecTests` |
| 2026-06-19 | Validation passed | `dotnet build`, `StormTelegramCallbackExecutableSpecTests` 1/1, `TelegramBotCallbackCoverageTests` 7/7 |

## Gate

Статус: `EXEC completed`

Переход в EXEC подтвержден пользователем фразой `Спеку подтверждаю`.
