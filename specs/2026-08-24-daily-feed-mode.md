...
| EXEC | CI regression: pre-open writer | 1.0 | Локально отсутствует stable SDK 10.0.400 | Commit/push и повторить authoritative CI | Нет | Нет | CI выявил только Windows-sharing issue в assertion теста: fallback корректно сохранял исходник, но test reader открывался до закрытия pre-open writer. Тест теперь пишет через этот handle, закрывает его и лишь затем читает источник/tombstone; независимый repeat review — `PASS`. | `DailyMarkdownStorageTests`, эта SPEC, PR #285 |
| EXEC | CI regression: task conversion teardown | 1.0 | Локально отсутствует stable SDK 10.0.400 | Commit/push и повторить authoritative CI | Нет | Нет | Повторный CI выявил latent test race: area UI-test ждал создания draft, а не конца conversion, и удалял vault во время Markdown write. Все три аналогичных test flows теперь ждут terminal `HasCreatedTask && !IsBusy`; два независимых анализа подтвердили, что production change/retry cleanup не требуется. | `FeedControlUiTests`, эта SPEC, PR #285 |
