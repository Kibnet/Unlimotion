...
Вариант 2:
1. Загрузите и установите [.NET 10.0 SDK](https://dotnet.microsoft.com/en-us/download) для своей операционной системы
...
При первом запуске новой модели старые активные задачи (`IsCompleted=false`) мигрируются в `Не готово`, потому что `Подготовлено` теперь означает явное решение пользователя о достаточности контекста. Откат миграции рассчитан на Git history каталога задач.
