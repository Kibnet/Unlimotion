...
Assert-Match $workflow 'Restore Android Project[\s\S]*for rid in android-arm64 android-x64[\s\S]*dotnet restore src/Unlimotion\.Android/Unlimotion\.Android\.csproj[\s\S]*-r "\$rid"[\s\S]*-p:RuntimeIdentifiers="\$rid"' 'android-packaging workflow must restore Android RID assets before packaging without repeating restore inside package builds.'
